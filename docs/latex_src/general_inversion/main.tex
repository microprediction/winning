...
produced at repository tag \texttt{paper-r2}, package version
\texttt{1.3.0}.}}
\date{August 2026}
...
against the accuracy floors reported below. The envelope must use the
...
$1-\rho_{\mathrm{first}(i)}$; with the root effect entering at its cophenetic value, the implied
correlation equals the floored cophenetic matrix. The shipped
\texttt{from\_linkage} instead fixes the root effect to zero, the root
being choice-irrelevant; the two models agree on contrast space, with
pairwise difference variances unchanged, but the zero-root model's raw
correlations are $(\rho_{ij}-\rho_0)/(1-\rho_0)$, not $\rho_{ij}$.
...
$q_\mu e_j$. The contribution at infinity vanishes for any integrable
$q_0$, and the only face of $\partial R_i$ whose outward normal has a
...
best effort silently. Ignoring known structure at inversion time is
...
discarded, and the alternation returns the best iterate. The objective
is nonconvex in the pair, so no global certificate is claimed; multistart
dispersion is a cheap diagnostic and the residual report exposes the
value reached.
...
unverifiable entry by entry. That set is also selected by the same
simulation that would have to certify it, so a fixed-set binomial
bound does not apply to it. The test that would apply is sample
splitting: fix the zero-win set on one half of the draws, count the
wins the independent second half places anywhere in it, and compare
the resulting interval for its total mass with the model's
$2$--$3\times10^{-4}$. We report the resolved bulk as verified against
this referee and the zero-win tail as untested by it; the
tail-exactness evidence at small $n$ comes from the Botev referee
instead.
...
The estimand is the penalized (MAP) maximizer under a Jeffreys Dirichlet
prior, counts $+\tfrac12$, applied identically to every arm. This is not
cosmetic. The unpenalized saturated likelihood has \emph{no} finite
...
$1.27$ and it is empty with probability $e^{-1.27}=0.28$; two of the
eight replications (seeds $103$ and $105$) do have an empty cell, where
an unpenalized optimizer returns a tolerance-dependent point on a
boundary sequence rather than a maximizer. Eight replications:
...
\midrule
exact gradient MAP & $0.0148$ & $2.4$ s \\
MSL, $R=100$ & $0.0275$ & $3.5$ s \\
MSL, $R=1000$ & $0.0161$ & $39.9$ s \\
\bottomrule
\end{tabular}
\end{center}

\noindent The exact path is both faster and more accurate than the
$R=100$ simulator here, and reaches the $R=1000$ accuracy in a
sixteenth of its time. We present the gap as a diagnostic rather than
an anchored constant: at eight replications the paired difference is
stable in sign but its magnitude carries replication noise we have not
driven down.
