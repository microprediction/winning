...
\begin{abstract}
We give a scalable engine for inference and counterfactual selection in
large correlated races, and specialize it to multinomial probit with
supplied low-rank-plus-diagonal covariance.
...
probabilities, matrix-free Jacobian--vector products, and every
single-removal counterfactual, in $O(nLQ)$
...
structures, and for trees exact within clusters). That Jacobian is a
weighted graph Laplacian whose edge weights are photo-finish tie
densities: the geometry of substitution between alternatives, delivered
by the same pass that prices them, so removal counterfactuals reuse the
field --- on a lattice certified to cover the post-removal winner, whose
bulk is the first- or second-finisher distribution of the original race
(the implementation checks the post-removal mass rather than
renormalizing a defect away).
At $n=200$ it is two hundred times faster than GHK at $10^4$ draws, with
...
These objects support not only discrete-choice estimation but also
selection and routing among large correlated populations of models,
experts, or autonomous agents.
\end{abstract}
...
and survival of $X_i$. One notational warning, once and loudly: from
here on $\mu_i$ is a TIME location --- raising it makes contestant $i$
worse and lowers $p_i$ --- the negative of the introduction's utility
location. Inversion returns time locations; utility coefficients are
their negation, and implementations that forget the sign flip invert
their leaderboards.
...
\end{equation}
The sibling messages sit OUTSIDE the parent-factor quadrature, and
correctly so, though it is not obvious: the parent effect shifts the
candidate and every sibling subtree by the same amount, so it cancels
from every candidate-versus-sibling comparison before those comparisons
are evaluated; only the cavity from outside the parent must be shifted
and averaged. Contestant $i$ then wins against its own leaf cavity,
...
contrast space. To prevent overreading: the theorem establishes
well-posedness, not convergence of this particular clipped iteration,
which is an empirically safeguarded solver --- the stress suite
converges everywhere tested, and a safeguarded convex or Newton--Krylov
fallback is available in principle should a case ever demand it. Round trips through the shipped front door at $n=400$
...
The fitting procedure, stated so it can be reproduced. The fit runs in
COVARIANCE coordinates, deliberately: an earlier draft standardized to a
correlation matrix, fit there, and restored scales afterwards, and a
referee supplied the two-line refutation. With $\Sigma=SCS$,
$S=\mathrm{diag}(s)$, the choice-relevant residual after restoration is
$PS(C-M)SP$, not $P(C-M)P$, and $P$ does not commute with an unequal
$S$: at $n=2$ with equicorrelated $C$ ($\rho=0.9$) and $s=(1,2)$, the
correlation-space quotient objective declares the fit $M=(1-\rho)I$
exact while the restored model gets the only identified quantity, the
difference variance, wrong by nearly a factor of three ($0.5$ against
$1.4$). Equivalently, the choice-irrelevant direction in correlation
coordinates is $S^{-1}\mathbf{1}$, not $\mathbf{1}$. A single global
rescaling for conditioning is harmless; per-coordinate standardization
is not, and the shipped test suite pins the analytic two-alternative
example and an exact-grammar round trip with variances spanning two
decades. So: fit $\Sigma$ itself. (i)~Global factors: the quotient fit of \S\ref{sec:jac}
...
against thirty-six for a $10^6$-draw simulation, agrees with it at its
noise floor, and prices the $82$ percent of tail alternatives for which
the simulation records zero wins.
...
and projected residuals throughout, the ensemble study below ran both
arms --- a raw top-$k$ eigenfit pipeline and the identified pipeline ---
over twenty seeds per ensemble. The measured outcome: on thirteen of
fifteen named ensembles the arms are statistically indistinguishable,
none favors the raw arm, and on block equicorrelation the identified
arm is eight times better at the median ($2.5\times10^{-3}$ against
$2.0\times10^{-2}$) with a worst seed of $3.2\times10^{-3}$ against the
raw arm's $0.162$: the raw eigenfit spends its rank on a near-common
component exactly as the identification argument predicts. The
ensembles the earlier draft reported as worse under projection (factor
with sparse links, hierarchical) are modestly \emph{better} under the
correct objective.
...
everything). To be unambiguous about the statistic: the TV columns are
FULL-vector total variation against the empirical frequency vector,
...
and prices the tails. The claim, stated so it cannot be read past its
evidence: for complete-vector pricing and inversion under the factor,
