...
$O(n^2 R)$ per probability vector with error $O(R^{-1/2})$. We observe that
...
nested and hierarchical structures, which are also the covariances
multinomial probit can identify---the field method is deterministic, computes
all $n$ probabilities and their exact Jacobian in $O(nLQ)$ operations over a
lattice of $L$ winning values and $Q$ factor quadrature nodes, and inverts
observed probabilities back to utilities. At $n=200$ it is two hundred times faster than GHK at $10^4$
...
The structures that admit this treatment cover the estimable cases.
...
folklore: Stata's \texttt{asmprobit} and \texttt{cmmprobit} compute
probabilities by GHK \citep{statacorp2023}, and R's \texttt{mlogit} and
\texttt{mvProbit} default to it \citep{croissant2020}. Two properties matter
...
approximations, and \texttt{Rprobit} implements it. Expectation propagation
...
$7\times10^{-8}$, which is within the tilting estimate's own error. Tail
accuracy is a property of the lattice, not of node placement, so the grammar
kernels price the tails that simulation cannot see.
