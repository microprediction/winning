...
Long-only portfolio weights are race probabilities, and there is a tree-race
covariance under which hierarchical risk parity's bisection geometry
...
Inversion recovers mean-zero $\mu$ from a target $p$ with the covariance
fixed and known: an adaptive damped fixed point on the log residual carries
the iterate into Newton's basin, then Newton steps on
$\log p(\mu)-\log p^\star$, preconditioned by the own-parameter slopes and
solved matrix-free, reach $10^{-8}$; targets below the forward map's
resolution are floored, and the recovered utilities for those entries are
one-sided bounds. Ignoring known structure at inversion time is
