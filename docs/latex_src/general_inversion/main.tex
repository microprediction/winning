...
The fitting procedure, stated so it can be reproduced. Standardize $\Sigma$
to a correlation matrix $C$. (i)~Global factors: the top-$k$ eigenpairs of
$C$, $k=3$ throughout. (ii)~Blocks: average-linkage clustering on
$\sqrt{(1-C)/2}$ cut to $20$ clusters; for each cluster, the leading
eigenvector of the within-cluster residual $C-VV'$ with its diagonal zeroed,
giving one rank-one loading per cluster (clusters are disjoint). Stages (i)
and (ii) alternate for eight sweeps, each stage refit with the diagonal
excluded so that factors never chase variance owed to the idiosyncratic
term. (iii)~Residual promotion: the top $m=5$ eigencomponents of what
remains, diagonal zeroed, appended as further factor columns; negative
eigenvalues are not promoted. (iv)~The idiosyncratic variances are
$D_i=\max(C_{ii}-\sum_r V_{ir}^2,\,10^{-3})$, so the assembled model is
positive semidefinite by construction as a sum of Gram terms and a
nonnegative diagonal. The loss at every stage is Frobenius error on the
off-diagonal entries. The constants $k$, the cluster count, the sweep count
and $m$ are fixed, not tuned per ensemble; a spectral rule for $m$ was
tested and rejected (it saturates its cap for marginal gains and does not
repair the failure case below). What results is a factor-plus-blocks race
priced in one deterministic call. At $n=2000$ the call takes five seconds
against thirty-six for a $10^6$-draw simulation, agrees with it at its noise
floor, and prices the $82$ percent of tail alternatives for which the
simulation records zero wins.

The fit is performed in raw correlation space. Because choices depend on
$\Sigma$ only through the contrast projection $P\Sigma P$, we also ran the
identical pipeline with stage (i) replaced by the projected fit
(\texttt{factor\_model\_contrast}): on these ensembles it is
indistinguishable on most rows and two to six times worse on several
(factor with sparse links, hierarchical, block equicorrelation, kernel).
The mechanism is instructive. Projection pays off when $\Sigma$ carries a
large choice-irrelevant common component, and none of these correlation
ensembles does; meanwhile several truths lie in or near the raw
rank-plus-diagonal class, and $P\Sigma P$ of such a matrix no longer does,
so projecting first manufactures model error where the raw fit had none.
The projected fit remains the right tool when a dominant common shock is
present.

Accuracy depends on the generating ensemble, so it is reported per named
ensemble of the \texttt{randomcov} library (one draw per ensemble at
$n=300$, seed fixed, judged on entries a two-million-path referee resolves;
the complete stratified table, all fifteen ensembles by probability band
with $\tfrac12\lVert\cdot\rVert_1$ and $\lVert\cdot\rVert_\infty$, is
generated by the committed \texttt{run\_ensembles3.py}). Representative
rows, absolute error:

\begin{center}\small
\begin{tabular}{lrrrrr}
\toprule
ensemble & $\tfrac12\lVert p-\hat p\rVert_1$ & $\lVert p-\hat p\rVert_\infty$
& med, $p>10^{-2}$ & med, $10^{-3}$--$10^{-2}$ & med, $<10^{-3}$\\
\midrule
factor $+$ sparse links & $2.8\times10^{-3}$ & $7.1\times10^{-4}$ & $1.2\times10^{-4}$ & $2.8\times10^{-5}$ & $6.5\times10^{-6}$\\
sparse-precision graphical & $4.8\times10^{-3}$ & $5.7\times10^{-4}$ & $3.2\times10^{-4}$ & $5.1\times10^{-5}$ & $5.8\times10^{-6}$\\
Wishart & $1.2\times10^{-2}$ & $3.2\times10^{-3}$ & $5.5\times10^{-4}$ & $1.1\times10^{-4}$ & $1.1\times10^{-5}$\\
residuals (dense-strong) & $6.0\times10^{-2}$ & $1.7\times10^{-2}$ & $2.3\times10^{-3}$ & $6.8\times10^{-4}$ & $5.8\times10^{-5}$\\
kernel (Mat\'ern/RBF) & $3.8\times10^{-2}$ & $1.3\times10^{-2}$ & $8.8\times10^{-3}$ & $4.3\times10^{-3}$ & $4.4\times10^{-4}$\\
\bottomrule
\end{tabular}
\end{center}

Eleven further ensembles (Marchenko--Pastur and spiked spectra, LKJ, onion,
vine, hierarchical, block equicorrelation, AR(1), elliptope walks, and two
others) fall between the first and fourth rows. The kernel row is the
failure case.
