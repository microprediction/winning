...
by the same pass that prices them, so counterfactual removals need no
fresh integration.
...
each seed judged by its own $10^6$-path referee on the entries the
referee resolves (the committed \texttt{run\_ensembles4.py} and
\texttt{run\_kernel4.py} regenerate everything). Total variation is
reported as median, ninth decile and worst over seeds; the last column
is the median per-entry absolute error on resolved entries.
