...
The cleanest evidence is internal: the same factor conditioning
computed one alternative at a time, on the same nodes and lattice,
agrees to $3\times10^{-15}$ and costs six hundred times more --- the
field, not the conditioning, is the contribution. Against the field
standard, at $n=200$ it is two hundred times faster than GHK at $10^4$
draws, with error below the comparison's resolution; GHK's measured cost grows superquadratically (exponent $2.15$ to $2.8$
...
and is quarantined in Appendix~\ref{app:extrap}, visually separate from
measured results by design. Every timing is regenerated by the committed
\texttt{bench.py}.
...
Everything in this appendix is an extrapolation of the measured cost
law of \S\ref{sec:num}, not a measurement, and is fenced off here for
exactly that reason: brackets like these invite argument about cache
effects, parallelism, Cholesky reuse and regime changes rather than
about the contribution. The lattice column is measured; the GHK column
extrapolates the two measured exponents ($2.15$ and $2.8$).
