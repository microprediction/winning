...
itself is never available, which forecloses everything that needs it. No pricing
benchmark is possible for the same reason: the method never produces the
probabilities.
...
behaves alike. Expectation propagation is the modern member of this family and
inherits its profile; we benchmark the family through its classical members.
...
integrator. The sequential-conditioning family is the only entry whose error
...
and R ports (Sobol and dependency-free Halton respectively, agreeing to
$5\times10^{-4}$ on the worst case), with the escalation pinned by
regression tests; the figures above are the pre-escalation diagnosis.
