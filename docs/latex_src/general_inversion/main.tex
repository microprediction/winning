...
known covariance (the Jacobian is exact for factor, block and nested
structures, and for trees exact within clusters). At $n=200$ it is two hundred times faster than GHK at $10^4$ draws, with
...
error is near $10^{-2}$; the lattice error is near $10^{-8}$. The bracketed
rows extrapolate far beyond anything GHK is actually run at; that is their
point. Nobody prices a million-alternative field with GHK, and the brackets
say what the cost law implies about why.
...
\]
linearly in $t$. Constancy along $\mathbf{1}$ is translation invariance
...
contrast space. Round trips through the shipped front door at $n=400$
...
to a correlation matrix $C=\mathrm{diag}(s)^{-1}\Sigma\,\mathrm{diag}(s)^{-1}$
with $s_i=\sqrt{\Sigma_{ii}}$; after fitting $C\approx VV'+\mathrm{diag}(D)$,
the covariance-scale model is restored as
$\mathrm{diag}(s)\,(VV'+\mathrm{diag}(D))\,\mathrm{diag}(s)$, that is,
loadings $s\!\circ\!V$ and idiosyncratic variances $s^2 D$, so nothing
below loses the original scales. (i)~Global factors: the certified quotient fit of \S\ref{sec:jac}
(\texttt{factor\_model\_projected}), $k=3$ throughout, returning $(V,D_0)$
that minimize the identified objective. (ii)~The working residual is then
...
floored at $10^{-3}$. The constants $k$, the cluster count and $m$ are
...
\texttt{cov=} directly), not a research script. At $n=2000$ the call takes five seconds
...
An earlier draft compared a raw-space fit against a fit of
$VV'+\mathrm{diag}(D)$ \emph{to} $P\Sigma P$ and found the latter worse
on several ensembles; a referee correctly identified that as a
comparison between the raw objective and the \emph{wrong} projected
objective ($P\Sigma P$ of a rank-plus-diagonal matrix is not
rank-plus-diagonal). With the identified objective of \S\ref{sec:jac}
and projected residuals throughout, the ensemble study below runs both
arms --- a raw top-$k$ eigenfit pipeline and the identified pipeline ---
and the identified arm no longer pays a penalty on in-class truths while
retaining its immunity to choice-irrelevant common components.
...
failure case, and the mechanism is the length scale, not the kernel. A
smooth kernel at long length scale has a rapidly decaying spectrum, which
is precisely what a low-rank-plus-diagonal fit wants; those draws sit at
the easy end of the table. At short length scale the covariance is
locality: correlation mass lives on neighbor pairs, the spectrum flattens,
and the rank needed to hold it grows with the number of correlation
lengths in the domain, which no fixed $k+m$ supplies. Mat\'ern roughness
decays polynomially, so at matched length scale it is the harder low-rank
target, but it is exactly the family with a sparse-precision escape
through the SPDE connection \citep{lindgren2011}; the squared exponential
lacks that route and instead becomes easy again as its spectrum
collapses. The right algorithm at short length scale is sequential
conditioning with cheap sparse-precision draws, and a production front
door should dispatch on the structure it detects.
...
and prices the tails. The claim is scoped to the task measured: pricing
and inverting complete choice-probability vectors under a known
structured covariance --- the inner loop that share inversion,
counterfactuals, and derivative evaluation repeat --- and for that task,
under the structured families applied multinomial probit imposes, we
consider GHK replaced. Estimation pipelines built around GHK are a
...
point simulation would have manufactured.
