...
multinomial probit can identify---one shared field computes all $n$
...
error below the comparison's resolution; GHK's measured cost grows as
$n^{2.8}$ against the lattice's near-linear growth, so the ratio widens
almost quadratically. The same two calls price and invert softmax,
...
and prices the tails. Identification restricts multinomial probit to
