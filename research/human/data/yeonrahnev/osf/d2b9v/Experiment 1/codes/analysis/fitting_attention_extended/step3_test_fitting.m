...
% '.../Experiment 1/data/fitting results/attention_extended'.
...
for subject = 1:length(modelFit)
    ...
    for stimPresented=1:nStim
        ...
        [~, att_item{stimPresented}] = sort(rand(N,4),2);
        att_item{stimPresented} = att_item{stimPresented}(:,1:nItem)';
        
        % select attended siganls and response
        attendedSignal(1,:) = relevantSignal{stimPresented}(sub2ind(size(relevantSignal{stimPresented}),att_item{stimPresented}(1,:),[1:N]));
        attendedSignal(2,:) = relevantSignal{stimPresented}(sub2ind(size(relevantSignal{stimPresented}),att_item{stimPresented}(2,:),[1:N]));
        if nItem == 3
            attendedSignal(3,:) = relevantSignal{stimPresented}(sub2ind(size(relevantSignal{stimPresented}),att_item{stimPresented}(3,:),[1:N]));
        end
    ...
    for stimPresented=1:nStim
        for stimPair=1:nStim
            if stimPresented~=stimPair %the pair needs to contain two different colors
                ...
                % When one of the attended items is the presented stimulus,
                % then it is correct
                numCorrect = numCorrect + ...
                    sum(sum(sum(cat(3,(att_item{stimPresented} == stimPresented), -(att_item{stimPresented} == stimPair)),3),1)==1);
                
                % When the paired stimulus get attention, then it is wrong
                numWrong = numWrong + ...
                    sum(sum(sum(cat(3,(att_item{stimPresented} == stimPresented), -(att_item{stimPresented} == stimPair)),3),1)==-1);
