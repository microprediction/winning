...
another day.'' Every citation is verified to
...
Three checks, all seeded and committed
(\texttt{research/equilibrium/exp2\_localtime}).
...
pair $(0,1)$, Monte Carlo $0.3539\pm0.0038$ against lattice $0.3544$
($z=-0.14$); pair $(0,3)$, $0.0946\pm0.0022$ against $0.0956$
($z=-0.47$).

\emph{The trace identity.} Summing named pairs gives the ranked
rank-$1$/rank-$2$ gap local time, and zero row sums make the total a
trace: $\sum_{i<j} w_{ij} = -\tfrac12\operatorname{tr}J$. Measured
identical to six decimals ($0.995801$ both ways). The trace is the sum
of the own-slopes the forward pass returns anyway, so the whole-field
collision rate is free with pricing.

\emph{Scale.} For a Russell-sized field, $n=3000$, the full expected
...
orders as everything above. Implementation is tracked as repository
issue~\#12; equation \eqref{eq:topk} is derived here and not yet
