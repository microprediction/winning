"""v4: v3 through the package's rust-backed race.

Same index, same two stored floats, same coverage math; the per-query
race now runs through winning.race_probabilities (min-wins: smallest
debiased distance wins), which dispatches to the fastrace kernel with
the winner-bulk window. v3's 42 ms/query overhead was the pure-python
quadrature; this measures the deployable number.
...
for i in range(N_Q):
    ...
    t1 = time.time()
    p = race_probabilities(s[cand], D=VAR[cand], points=129, window="span")
...
# matched-achieved-coverage verdict: cost of adaptive vs fixed depth at
# the coverage the adaptive rule actually delivers
print("\nmatched coverage:", flush=True)
for d in DELTAS:
    md = np.array(depth[d]); cov = np.mean(hit[d])
    # fixed depth needed for the same achieved coverage (from true ranks)
    m_fixed = int(np.quantile(ranks, cov, method="higher")) + 1
    print(f"  achieved {cov:.3f}: adaptive mean m {md.mean():7.1f}  "
          f"fixed-m needed {m_fixed:5d}  saving {m_fixed/md.mean():4.1f}x",
          flush=True)
