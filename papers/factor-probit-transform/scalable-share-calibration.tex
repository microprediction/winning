...
computed shares agree with higher-resolution references of the same
construction to a few parts in a hundred thousand in log share,
hundred-to-one shares included. Independent
RQMC inversion checks through $N = 200$ recover utilities at the
reference error floor. The established high-precision evaluators price
one orthant at a time. A per-alternative factor-conditioned RQMC
baseline computes the same share vector roughly $140\times$ slower at
matched accuracy at $N = 1000$ on the reported hardware, and the gap
widens linearly in $N$ because it performs $N$ separate integrals.
\end{abstract}
...
This paper solves a calibration problem. The data are $N$ shares
(choice frequencies, market shares, win probabilities), one per
alternative.\footnote{We call the choice probabilities $p_i$
\emph{shares} throughout. The racing origin \citep{cotton2021} called
them \emph{state prices}, borrowing prediction-market and finance
usage. In the probit choice setting \emph{shares}, as in market shares,
is the more natural term. \emph{Probability} would also be imprecise on
the discrete lattice used later, where ties have positive probability
and a dead heat splits the payout. The lattice objects really are state
prices.} The model is multinomial probit: jointly Gaussian utilities
whose covariance is low-rank-plus-diagonal,
\[
\Sigma = VV^\top + \mathrm{diag}(D),
\]
a race in which each performance is idiosyncratic but also influenced
by a small number of factors common to all runners. The factor
structure $(V, D)$ is fixed and supplied. The problem is to recover the
utilities at which the model reproduces the shares, together with the
derivatives and removal counterfactuals that make the calibrated model
usable, at $N$ in the thousands.\footnote{In racing parlance, we are
recovering the relative ability of horses from the bookmaker odds, even
when some horses are known to be correlated with others. The original
transform \citep{cotton2021} assumed independent performances.}

The logit version of this inversion is daily routine. Demand estimation
in the Berry--Levinsohn--Pakes (BLP) tradition begins by inverting
observed market shares to mean utilities, and runs on the closed forms
and contraction mapping that the Gumbel distribution supplies
\citep{berry1994,berry1995,conlon2020}. The probit version is treated
as out of reach. The record is explicit, and we quote it before
removing the barrier for the covariance class above.

The model is among the oldest in choice theory. Thurstone's 1927 law
of comparative judgment posited jointly Gaussian utilities, correlated
in its general cases \citep{thurstone1927}. Transportation science
pursued its computation early \citep{daganzo1977}, and econometrics
adopted the same object as the conditional, or multinomial, probit
model (MNP) \citep{hausman1978}. It re-emerges wherever latent
performance is Gaussian, from bioassay and quantitative genetics to
item response theory, signal detection, credit risk, skill rating, and
the final layer of heteroskedastic image classifiers
\citep{collier2021}. Section~\ref{sec:applications} collects these
fields. The construction below applies to those of their problems in
which the observed event is a mutually exclusive argmax, or
first-arrival, among the $N$ alternatives. Any empirical
argument for one race over another on real data is deferred to a
companion survey \citep{cottonsurvey}. Here the focus is the
computation, and the record of its difficulty, which is clearest in
psychometrics and econometrics.

Psychometrics stated the backward problem and abandoned it. Torgerson's
text records the degrees-of-freedom count, $n$ observed frequencies
against $n$ scale values, so ``we ought, therefore, to be able to fit
perfectly''. It also records the retreat that followed. Every working
method from Thurstone forward, down to modern Thurstonian rating
engines such as TrueSkill, reduces the $N$-way contest to pairwise
events \citep{torgerson1958,herbrich2006}.
% VERIFIED 2026-08-24 against the archive.org scan: exact sentence p.194, df discussion pp.193-194. See quote-verification.md.

Econometrics kept the $N$-way event and changed the distribution. A
major reason for logit's dominance is brutally practical. Logit models
can be computed. Probit probabilities are $(N-1)$-dimensional Gaussian
integrals with, generically, no closed form. The classic econometric
evaluator is the Geweke--Hajivassiliou--Keane (GHK) simulator
\citep{geweke1994,hajivassiliou1998,train2009}, the econometric branch
of the Genz separation-of-variables family \citep{genz1992}. It treats
the probability that alternative $i$ beats the rest as an orthant
probability of the difference utilities, estimated by sampling
truncated normals along a Cholesky factor. It is elegant and
stochastic, its accuracy improves only as more Monte Carlo draws are
added, and it prices one orthant per alternative, exploiting nothing of
the product shared across the $N$ winner events.
Graduate students are warned off accordingly. Conlon's industrial
organization lecture notes put unrestricted MNP at ``potentially $J^2$
parameters (that is a lot)!'' and one simulated integral of dimension
$J$ per share evaluation: ``I wouldn't do this for $J \geq
5$.''\footnote{C.~Conlon, graduate industrial organization lecture
notes (multinomial choice), New York University,
\texttt{github.com/chrisconlon/Grad-IO}.}
% VERIFIED 2026-08-24 against multinomial_choice2.pdf (Fall 2025), slide 3: both quotes verbatim (second is two consecutive bullets). See quote-verification.md.
The reference implementation of the logit tradition says the same by
omission. The word \emph{probit} does not occur in the source tree of
PyBLP \citep{conlon2020}, whose consumer heterogeneity is a rank-$K_2$
factor loading on observed characteristics. A tensor-product rule with
$m$ nodes per dimension there costs $m^{K_2}$ nodes, and the accounting
in \citet{conlon2020} uses $I_t = 4^{K_2}$, which is 65{,}536 nodes at
$K_2 = 8$.

The inversion is harder still, and the literature is quantitative about
why. The inverse share map amplifies whatever simulation and sampling
error the shares carry. In the large-product asymptotics of Berry,
Linton and Pakes, even consistency places a growth requirement on the
number of Monte Carlo draws behind the simulated shares, and asymptotic
normality requires the number of draws to grow as the square of the
number of products \citep{berry2004}. Freyberger shows that unless the
number of simulation draws grows quickly enough relative to the number
of markets observed, the simulation error that survives shows up as a
first-order bias in the estimated parameters, not merely as additional
variance \citep{freyberger2015}. The standard
escape at large $N$, estimation on a sampled subset of alternatives,
rests on a logit cancellation, an explicit use of independence from
irrelevant alternatives (IIA), and does not transfer to probit in the
same closed form \citep{train2009}. Commercial conjoint practice,
meanwhile, has simulated a closely related factor race since 1999.
Randomized First Choice perturbs part-worths with normal errors shared
across alternatives, inducing exactly the low-rank-plus-diagonal
covariance, over an idiosyncratic term that is Gumbel in its logit mode
and normal in its probit mode \citep{huber1999}. The variance split is
tuned by grid search rather than fitted, and target shares are matched
with an ad hoc fixed point its own authors call ``fudge factors'' and
decline to encourage \citep{orme2006}.
% VERIFIED 2026-08-24 against the Sawtooth PDFs: HOM 1999 p.2 glossary and Orme-Johnson 2006 p.2 verbatim. See quote-verification.md.

The standard reason for needing none of this is the McFadden--Train
theorem that mixed logit approximates any random-utility model
arbitrarily closely \citep{mcfaddentrain2000}. The construction is priced in its
own paper. It sends the extreme-value scale to zero while the mixing
distribution grows without bound, and the authors report ``no practical
indication of how to choose parsimonious mixing families, or how many
terms are needed.'' The Gaussian kernel they considered was set aside
on computational grounds, not fidelity. The limit is real. The question
is whether the required rank is affordable, and that is a computational
question, not a theoretical one.

The field kept the computable models and gave up the model it started
from. The concession is not free. One measure of the cost comes from an
oracle factor-probit benchmark with every marginal variance held at one
...
scripts in the repository; see the Reproducibility section.} A
companion factorial with a deliberately misspecified truth tells a
similar story.

With unrestricted alternative intercepts, a single interior share
vector cannot distinguish full-support random-utility models, because
each can be inverted to match it. However, when logit, probit, or other
models are calibrated to observed shares, the recovered utilities can
serve as inputs to questions the data do not answer directly:
elasticities, welfare, pricing, the effect of adding or removing an
alternative, and combinatorial probabilities. It is at this point that
they make different, falsifiable predictions.

This paper's sole point is that probit's computational barrier is not
real for this covariance class. We solve the fixed factor-probit share
calibration problem at $N = 1000$ alternatives in three seconds at rank
two with the compiled implementation, and ten thousand alternatives
stay under a minute.\footnote{Estimating $(V, D)$ is the outer problem.
It is discussed in Section~\ref{sec:discussion} but not claimed here.}
...
The exact map is invariant to orthogonal rotations $V \mapsto VO$ and
to common shifts $V \mapsto V + \mathbf{1}c^\top$, which move every
utility by the same factor-dependent amount. The finite implementation
is not automatically invariant to either. The envelope, the warm
start, and the step caps are built from the raw conditional means and
marginal variances, and a tensor-product factor rule at finite order is
not rotation-invariant. A supplied $V$ carrying a large common
component should therefore be centered by the caller, $V \mapsto (I -
\mathbf{1}\mathbf{1}^\top/N)V$, and rotated to a fixed singular-value
orientation. The forward pass does not re-canonicalize what it is
given. The sensitivity is small at production resolution: in a spot
check at $N = 30$, $k = 2$, a common shift of ten factor standard
deviations moved log shares by under $10^{-9}$ and a rotation by under
$10^{-6}$.
...
\State if $N = 2$: return the closed-form inverse, in which $\hat p_1$
pins $a_1 - a_2$ through one univariate normal CDF with variance $D_1 +
D_2 + \|v_1 - v_2\|^2$ (the loop below is a local two-cycle at $N = 2$;
Proposition~\ref{prop:jacobi})
\State $a \gets$ high-accuracy independent-race inversion using
marginal variances $D_i + \|v_i\|^2$ (this same algorithm with a
single zero factor node; at $V = 0$ the warm start is the analytic
log-share start, so the recursion is one level deep) \Comment{warm
start}
...
potential in the proof of Proposition~\ref{thm:inv}, so the problem
admits globally safeguarded convex optimization. A damped
Newton--Krylov method with a line search converges for the continuum
map under the usual descent and regularity conditions. We ship the
...
At $N = 2$ the photo-finish graph is $K_2$, which is bipartite, so
$\lambda_2 = 2$ exactly and the undamped update is a local two-cycle.
The oscillation holds the residual nearly constant, which also defeats
the growth-based damping safeguard. The implementation therefore
returns the closed-form $N = 2$ inverse before the loop, and the
general-base inversion fixes its damping at $0.7$ when $N = 2$.
...
2.4\times10^{-8}$ equals it, consistent with lattice truncation
contributing below that level. Signed grid errors can cancel in the
sum, so the equality is evidence rather than a bound.
...
\citet{lin2026} estimate perturbed utility models by minimizing the
associated Fenchel--Young loss, whose gradient in the utilities is
exactly $p(\mu) - \hat p$; the forward pass here supplies that
gradient for the factor probit case.
...
model, deterministically and sampling-free. Every one of these
orthant-probability methods prices one
...
and \citet{dunnett1989} is its standard algorithm. Credit risk
institutionalized the same reduction. The Basel II capital formula is
Vasicek's one-factor conditional-independence integral
\citep{vasicek2002,gordy2003}, adopted for regulation in preference to
portfolio simulation. The ranking-and-selection
...
The first-choice integral itself is psychometric and old.
\citet{thurstone1945} evaluated the survival-product integral
numerically for $n$ alternatives with a discretised computing formula,
noting that it carries no restriction on the shapes of the
distributions. The direction was forward only. \citet{guilford1937}
stated the backward problem at $N > 2$ and solved it with a composite
standard that \citet{torgerson1958} shows is Bradley--Terry in
disguise. The retreat to pairwise that followed is quoted in the
introduction.
...
causes off a single survival product nonparametrically. The earliest
instance we have found is older still. \citet{lambert1975} computed the
Barlow--Proschan importance of every fault-tree event from one shared
product, noting that the top-event probability need not be recomputed
each time. The deletion ensemble has a competing-risks ancestor as
well. Chiang's partial crude probabilities \citep{chiang1961} give
every deleted-subset share in closed form under proportional hazards,
which is the logit-trivial case. None of these embeds the assembly in
factor quadrature, differentiates it, or inverts it. Racing arrived at
it independently.
...
generalized-entropy duality, without an algorithm. For the Gaussian
case specifically, \citet{andersonghurye1977} and \citet{mukherjea1990}
prove that the distribution of the maximum identifies the parameters of
a multivariate normal. These are theorems without numerics.
\citet{li2018} comes closest to an algorithm. For arbitrary non-atomic
bases he writes the inversion as an unconstrained convex minimization
whose objective gradient is the share map, with globally convergent
superlinear methods. He supplies the formulation and the optimizer
without a fast evaluator of the shares and their Jacobian at large $N$.
The shared field supplies the evaluator. The two are complements, and
together they are the large-$N$ inversion for general bases.
...
cousin changes. This particular example is roughly a
six-order-of-magnitude reduction against plain winner-frequency Monte
Carlo. The figure is a back-of-the-envelope extrapolation, not a
matched benchmark against modern RQMC or specialized Gaussian
integration.
...
covariance} axis. Experiment 29 above separately shows that a large
substitution effect remains when every marginal variance is held at
one. Its design differs from the factorial in more than the variance
confound, so the two effect magnitudes are not directly comparable.
...
$\Sigma$, and modern Gaussian-probability methods reach high accuracy
one orthant at a time \citep{botev2017,fasano2025,cao2025}. The shared
field owns the factor form, all $N$ shares jointly, derivatives, and
calibration. Our experiments give some guidance as to the boundary. 
...
The engine was canonized independently, under different names, across
the quantitative sciences. Thurstone's comparative judgment, the
ordered and binary probits of applied microeconometrics, and the
Lazear--Rosen tournament \citep{lazear1981} are the same Gaussian
latent-performance model, and machine learning runs on the same family:
a softmax output layer is its Luce--Gumbel member
\citep{luce1959,yellott1977}, while Thurstone--Mosteller pairwise
comparison \citep{mosteller1951}, the probit member, underlies rating
systems and preference learning. Bioassay named the probit and made
...
computational stand-in \citep{lord1952}. Signal detection's $d'$ is a
...
factor \citep{vasicek2002,gordy2003}, a rank-one factor model of
exactly the form used here. Machine learning made probit its tractable
Bayesian classifier \citep{albertchib1993} and ships a Thurstonian race
as its skill-rating engine \citep{herbrich2006}. Wherever performance
is latent and Gaussian, the same object appears.
...
factors, and observing a candidate then re-ranking the rest is the
removal counterfactual computed here whenever the observation leaves
the other candidates' scores unchanged.
...
A companion working paper develops the correction
\citep{cotton2026polish}. It reads index weights as the win
probabilities of a race among assets, inverts them to latent abilities,
and re-tilts the allocation under a view on tail dependence, with the
calibration developed here as the inverse step.
...
and measured times, scores, and energies are modeled with Gaussian
errors far more often than with Gumbel. The Gumbel race is usually chosen backwards,
...
\bibitem[Anderson and Ghurye(1977)]{andersonghurye1977} T.~W. Anderson,
S.~G. Ghurye. Identification of parameters by the distribution of a
maximum random variable. \emph{Journal of the Royal Statistical Society
B} 39 (1977) 337--342.
...
\bibitem[Berry et~al.(2004)Berry, Linton, and Pakes]{berry2004} S.~Berry,
O.~B. Linton, A.~Pakes. Limit theorems for estimating the parameters of
differentiated product demand systems. \emph{Review of Economic
Studies} 71 (2004) 613--654.
...
\bibitem[Chiang(1961)]{chiang1961} C.~L. Chiang. On the probability of
death from specific causes in the presence of competing risks.
\emph{Proceedings of the Fourth Berkeley Symposium on Mathematical
Statistics and Probability} IV (1961) 169--180.
% VERIFY: article title from memory; volume/pages verified against prior-art memo.
...
\bibitem[Cotton(2026a)]{cotton2026polish} P.~Cotton. Thurstone
portfolio polishing: tail-sensitive Black--Litterman and beyond.
Working paper (2026),
\url{https://allocation.microprediction.org/papers/thurstone-portfolios/}.

\bibitem[Cotton(2026b)]{cottonsurvey} P.~Cotton. Probit versus logit:
...
\bibitem[Freyberger(2015)]{freyberger2015} J.~Freyberger. Asymptotic
theory for differentiated products demand models with many markets.
\emph{Journal of Econometrics} 185 (2015) 162--181.
...
\bibitem[Guilford(1937)]{guilford1937} J.~P. Guilford. Scale values
derived from the method of choices. \emph{Psychometrika} 2 (1937)
139--150.
...
\bibitem[Huber et~al.(1999)Huber, Orme, and Miller]{huber1999} J.~Huber,
B.~Orme, R.~Miller. Dealing with product similarity in conjoint
simulations. \emph{Sawtooth Software Conference Proceedings} (1999).
...
\bibitem[Lambert(1975)]{lambert1975} H.~E. Lambert. \emph{Fault Trees
for Decision Making in Systems Analysis}. PhD thesis, UCRL-51829,
...
\bibitem[Lin et~al.(2026)Lin, Yin, and Liu]{lin2026} X.~Lin,
Y.~Yin, T.~Liu. Fenchel--Young estimators of perturbed utility
models. arXiv:2602.21376 (2026).
...
\bibitem[McFadden and Train(2000)]{mcfaddentrain2000} D.~McFadden,
K.~Train. Mixed MNL models for discrete response. \emph{Journal of
...
\bibitem[Mukherjea and Stephens(1990)]{mukherjea1990} A.~Mukherjea,
R.~Stephens. Identification of parameters by the distribution of the
maximum random variable. \emph{Probability Theory and Related Fields}
84 (1990) 289--296.
% VERIFY: article title from memory; journal/volume/pages verified against prior-art memo.
...
\bibitem[Orme and Johnson(2006)]{orme2006} B.~Orme, R.~M. Johnson.
External effect adjustments in conjoint analysis. Sawtooth Software
...
\bibitem[Thurstone(1945)]{thurstone1945} L.~L. Thurstone. The
prediction of choice. \emph{Psychometrika} 10 (1945) 237--253.
...
\bibitem[Torgerson(1958)]{torgerson1958} W.~S. Torgerson. \emph{Theory
