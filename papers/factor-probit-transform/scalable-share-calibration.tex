...
The model itself is the oldest in choice theory. Thurstone's 1927 law
of comparative judgment posited jointly Gaussian utilities with
unrestricted correlation \citep{thurstone1927}. Econometrics
reintroduced the same object as the conditional, or multinomial, probit
...
integrals with, generically, no closed form. The standard evaluator is
the Geweke--Hajivassiliou--Keane (GHK) simulator
...
factor loading on observed characteristics, priced by its own
documentation at $4^{K_2}$ product-rule quadrature nodes, which is
65{,}536 nodes at $K_2 = 8$.

The inversion is harder still, and the literature is quantitative about
why. Berry, Linton and Pakes show that estimation through inverted
shares requires simulation draws growing as the square of the number of
products, because the inverse share map amplifies simulation and
sampling error \citep{berry2004}. Freyberger shows the surviving
simulation error enters the estimator as a first-order bias
\citep{freyberger2015}. The standard escape at large $N$, estimation on
a sampled subset of alternatives, rests on a logit cancellation, an
explicit use of independence from irrelevant alternatives (IIA), and
has no probit counterpart \citep{train2009}. The dominant commercial
conjoint platform, meanwhile, has shipped exactly this
low-rank-plus-diagonal Gaussian race since 1999 \citep{huber1999},
evaluating it by brute-force simulation with hand-tuned variance
scalars, and matching target shares with an ad hoc fixed point its own
authors call ``fudge factors'' and decline to encourage
\citep{orme2006}.
