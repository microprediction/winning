...
in three seconds at rank two. The computation is the probit analogue of a far more popular logit inversion. For example, 
...
The inverse
problem is also globally identified and onto the simplex interior
...
theory is stated in the natural \emph{max-wins} coordinates $\mu$. It
keeps the loadings' sign. The reflection is valid
...
identification, the well-posedness of its inverse, the graph that
...
are used only for $k > 4$. They are reproducible by construction with a fixed seed, though
...
\midrule
16 & 20--1000 & 2; 1.0/1.5 & $s = 1.0$ at $N \le 200$, else $1.5$ & twin $2{\times}10^6$-draw MC; $5{\times}10^6$ for inversion \\
...
$3\times10^{-13}$. The full identity is verified to $3\times10^{-9}$. That $G^*$ is an assignment problem is previously known per
...
differentiates logit BLP automatically. On the GEV side, closed-form
...
the use of probit models versus their analytically simpler logit
cousin. This particular example is roughly a six-order-of-magnitude speedup. 
...
$0.009$ for deleted shares above ten percent, $0.275$ against $0.031$
at 2--10\%, and $0.325$ against $0.113$ at 0.5--2\%, with factor
probit winning all twenty paired seeds in each of those strata. Below
...
of unequal marginal variances. We vary two things one at a time in a
...
zero loadings reproduces the Luce model exactly
\citep{luce1959,yellott1977}, yielding correlated-softmax
generalizations.
...
The territory is larger than econometrics. That's because a probit model is literally a race where performances
...
predicting market share is fundamental to company analysis many levels
higher. Countless choices are made by man, nature, and machine. Across all the world's applications
of large language models, perhaps a billion next-token choices are made a second. Those microscopic battles
determine the grand race for AI model supremacy.
...
table and figure; this submission is built from the tag
\texttt{jcgs-v2}. (Archive supplied as a zip with the submission;
repository at \url{https://github.com/microprediction/winning}.)
...
a survey of empirical accuracy. In preparation, 2026.
