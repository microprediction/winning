...
\begin{abstract}
\noindent When a probability vector over mutually exclusive outcomes has to be transported to a smaller
support, because an outcome is withdrawn, scratched, delisted, disallowed or masked out of a
softmax, proportional renormalization is the reflex, and in many models the architecture. An analyst holding shares and no restricted-menu observation has two
...
Case V. Any standardized noise shape fixed in advance gives such a map, and these two are the
conventional choices: the first is the reflex, the second the classical Thurstone model. Psychology has
tested renormalization under
...
This is proportional renormalization. The invariance it imposes is Independence of Irrelevant
...
The two could be termed \emph{linear normalization} and \emph{Gaussian renormalization}. Both
take the same shares and return shares over the survivors. The first holds the ratios of the
...
Table~\ref{tab:scoreboard} states the result before the argument for it. Thirty-seven
populations have full-menu shares that calibrate both maps and restricted-menu choices that
score them by held-out log loss. Section~\ref{sec:empirical} gives the protocol, the datasets
and the null.
...
Consumer goods, experiment 1 & observed menus & $+.0140$ & $.005$ & Costa-Gomes \\
...
Consumer goods, experiment 2 & observed menus & $+.0059$ & $.129$ & Costa-Gomes \\
...
\bottomrule
\end{tabular}
\caption{Held-out log loss of linear normalization minus that of Gaussian renormalization, in
...
computed. Forced-choice subgroups are quoted for the consumer experiments, since no deferral
coding enters those. Three further populations appear in Section~\ref{sec:boundary} rather than
here, because published aggregates fix the direction of the failure without supporting this
protocol.}
\label{tab:scoreboard}
\end{table}

Gaussian renormalization has the lower held-out log loss in twenty-eight of the thirty-seven
rows. That count is a description of this table and not an estimate of any rate, because the
rows are not independent, the families contribute unequal numbers of them, and the collections
were assembled by searching for restriction data rather than sampled from a population of
applications. The effect sizes and their families are what the table is for.

Eight of the nine rows where linear normalization predicts better are one phenomenon. Tones,
lines and the Getty condition whose four survivors are mutual confusions all place the
alternatives on a single perceptual dimension, and Section~\ref{sec:boundary} treats them
together. The ninth is Wikipedia, at $-0.0001$.
...
Debreu, and Rouder reaches the same diagnosis from the opposite direction.
...
are the conventional ones. Gumbel gives renormalization, which is the reflex and the
...
surviving. For interior share vectors and a smooth inverse share map the first generic
departure of the recalibrated restriction map from Case V is therefore cubic in $\varepsilon$.
...
of the $K-1$ utility differences carries a further $K(K-1)/2 - 1$ parameters, and no
combination of them changes the full-menu shares once the locations are free to adjust. They
are therefore unidentified from one full-menu share vector for every $K \ge 3$. A correlated model is available to anyone with
...
here, and a fitted shape or mixture parameter would beat both. That over-prediction is Yeon and
...
Over $1{,}296$ cells and $49$ subjects, linear normalization predicts better in every split.
...
alongside linear normalization rather than passed over. Four questions decide whether that condition holds. None of them is
...
A yes to the first two says linear normalization or a fitted correlated model rather than
...
Choosing the better of these two maps is therefore a smaller improvement than fitting a shape
parameter would be, wherever a restricted-menu observation exists to fit it on. The
...
Gaussian Case V is a credible, tuning-free alternative to proportional renormalization. Across
the collections examined it frequently produces useful odds contraction, with the most
persuasive evidence coming from experiments in which the same observer and stimulus are
evaluated before and after an unanticipated menu restriction. On those collections it is
typically as viable as linear normalization and often more so, which is all that is required of
it.
...
its standardized noise shape, and both are wrong in the same direction wherever the truth can be
measured, by more than they differ from one another. A fitted shape or mixture parameter beats
both wherever a restricted-menu observation exists to fit it on. A common noise scale cannot,
being a gauge.
