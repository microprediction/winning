...
One direction for future work is worth recording, because it would turn a limit into a
correction. At finite $\sigma/\beta$ the composite noise is Gaussian plus Gumbel, and the ratio
$\varepsilon = \beta/\sigma$ is a small parameter multiplying a fast, mean-reverting-like
component around a slow Gaussian one. That is the structure singular perturbation methods are
built for, as in fast mean-reverting stochastic volatility, where Case V would play the role
of the leading-order model and the first correction in $\varepsilon$ would predict departures
from it. The residuals this paper measures, observed contraction against the Case V
prediction, are precisely what such an expansion would have to explain, and we do not attempt
it here.
