...
then an empirical question. Two of the answers are measured below.
...
one master set of five letters with three nested response sets, blocked and counterbalanced,
and found the rule a fair first approximation that fails
...
Wills, Reimers, Stewart, Suret and McLaren \cite{wills2000} reject the rule in
categorization and argue for a Thurstonian replacement, citing Yellott. Their Experiment 2
withdraws a response category, and their data is deposited, so it is scored in
Section~\ref{sec:townsend} with the rest.

We revisit this question. The two parameter-free maps are calibrated on full-menu shares
alone, then scored on restricted menus neither has seen, against a null in which the axiom
holds by construction. Three things make that possible now and did not before. Trial-level
archives are open, so restricted menus can be scored on the observations that produced them
rather than on printed marginals. Inverting the $K$-alternative contest from shares alone is a
single pass over a shared grid \cite{cotton2021}, so the Gaussian map costs nothing to apply.
And out-of-sample scoring against a generative null is standard. Contraction of a noisy share
vector lowers log loss whether or not the axiom fails, and no comparison of fit indices
detects that.

The question has also left psychology. Masking a softmax and rescaling the survivors is
Equation~\eqref{eq:luce} applied to a restricted set, so the rule Clarke tested on letter
confusions in 1957 now runs inside models that make no reference to Luce.
