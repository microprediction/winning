...
One other assumption answers the question without fitting anything. Treat the shares as the
winning probabilities of a contest among independent Gaussian performances, recover the
locations that produce them, and re-run the contest among the survivors. That is Thurstone's
Case V. Section~\ref{sec:maps} shows these two are the only members of the independent
additive random-utility class that cost nothing to apply, because fixing any other noise law
requires a number the shares cannot supply. They make different predictions from the same
input, so they can be scored against each other.
