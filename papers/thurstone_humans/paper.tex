...
occur, proportional renormalization is exactly right: it is Bayes. Told instead that one
has been \emph{withdrawn}, scratched from the race, removed from the menu, masked out of a
softmax, renormalization is no longer a theorem but a guess. It is the guess almost
everyone makes. The alternative reads the probabilities as winning probabilities in a
contest of Gaussian performances and re-runs the contest over what remains, fitting
nothing. It forecasts better on an experiment where subjects chose from all thirty-one
subsets of five goods, and on twelve of twelve ranking collections. Contraction of a noisy
estimate also lowers log loss on its own, so we check against populations that renormalize
exactly; the advantage survives except in the smallest collection.
\end{abstract}
