...
The other two are different. In the third, the feasible set is intervened upon. A horse is
scratched, or a response option is disallowed, and the experiment that generates the choice
changes. The original share vector then does not determine the new shares. Linear and Gaussian
renormalization are two conventional answers, among the many that any fixed noise shape supplies,
and this paper is about that case. In the fourth, a model recomputes its scores with the permitted
set as an input. That is not a transport of the old vector at all, so neither map describes it.

A masked softmax belongs to the second case as arithmetic and to the third as a question. The
operation is the linear transport by construction, so there is nothing to choose. Whether it gives
the right answer depends on which of the two situations it stands for, and wherever the removal is
an intervention rather than a conditioning, the linear transport is already in place without
