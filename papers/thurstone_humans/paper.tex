...
population shares, and the alternatives are distinct unordered items, Gaussian renormalization
deserves to be tried alongside linear normalization rather than passed over. Four questions decide whether that
...
with the leader surviving, that the two maps agree to within the noise? A yes to the first three says renormalization or a
...
\emph{Gaussian renormalization}, to the status of an equal partner to the ubiquitous practice
of linear renormalization of probability. It requires no estimation and no tuning, and it is
the only other candidate with that property that is already conventional.

On the collections examined, Gaussian renormalization is at least the equal of linear
normalization, and often the better of the two, which is all that is needed to treat it as a
partner rather than a curiosity. Gaussian renormalization has the lower held-out log loss in
twenty-eight of the thirty-seven rows of Table~\ref{tab:scoreboard}, and in most of those it
also departs from a null in which the constant-ratio rule holds by construction. The row count
is descriptive: the rows are not independent and were not sampled from any population of
applications.
...
the same alternatives before and after. Or the withdrawn alternatives carry negligible mass while
the dominant alternative survives, in which case the two maps agree to within the noise. The within-set confusion mass of
...
than they differ from one another. A fitted shape or mixture parameter beats both wherever a restricted-menu
observation exists to fit it on, since a common noise scale is a gauge and cannot.

The same operation recurs outside choice modelling. Masking a softmax over fixed logits and
rescaling the survivors is linear normalization, so any model that restricts its output
vocabulary at inference time behaves as the constant-ratio rule prescribes, whatever mechanism
generated the logits.
