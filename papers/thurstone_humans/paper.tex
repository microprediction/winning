...
The gains are a few thousandths of a nat. Both maps are given the same probabilities, and
those probabilities carry most of the predictive content of a restricted menu, so the
scale of any difference between the two maps is set by the measurement rather than by the
maps.

To put a denominator under it we score a third forecast, one that does use restricted-menu
data: the training-fold frequency with which each item is top within each subset, smoothed
as above. Its loss reduction relative to renormalization is the gap in
Table~\ref{tab:gap}, and the race's share of that gap is the last two columns.
