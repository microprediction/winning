...
collections. Both results survive a null in which the axiom holds by construction,
which is necessary because contracting a noisily estimated share vector is also
shrinkage. The recommendation is narrow: when mapping aggregate full-menu shares to
...
The direction of the disagreement is a theorem. With the highest draw winning, let
$r = f/F$ be the reverse hazard of the common noise. For nested menus $T \subset S$ and
$i,j \in T$ with $a_i > a_j$, if $\log r$ is concave then the odds of $i$ against $j$
are at least as large in $S$ as in $T$, strictly so when the concavity is strict and at
least one contestant is actually removed. The argument is a covariance inequality
between two increasing functions.
...
We use the proposition for the sign and nothing else. It orders no magnitudes, and
contraction is not a property of a noise law: it depends on the share configuration, and
laws recalibrated to the same shares reorder between configurations. Heavier tails do
not reliably contract less.
