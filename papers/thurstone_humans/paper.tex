...
Gambles over token prizes & observed menus & $.035$--$.667$ & win$^{*}$ \\
US state ballots 2024 & observed ballots & $.005$ & win \\
Consumer goods, experiment 2 & observed menus & $.129$ & draw \\
\bottomrule
\end{tabular}
\caption{Thirteen wins, two draws, no losses. The starred row is one experiment run in three
frames differing only in how much arithmetic is needed to value an alternative: the race
wins where five operations are required, tail $.035$, and not where one is, tail $.667$.
Section~\ref{sec:more} argues this is the thesis working rather than failing. The upper block reads subset choice off a
complete ranking; the lower block observes it. Tail frequencies are floored at $1/201$ by
two hundred replicates. Forced-choice subgroups are quoted for
