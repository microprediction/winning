...
$\sigma/\beta$ the composite noise is Gaussian plus Gumbel, so $\varepsilon = \beta/\sigma$ is a
small parameter attached to a subordinate component sitting on a Gaussian leading order. That
is the setting for a singular perturbation expansion, with Case V as the leading-order
restriction map and the first term in $\varepsilon$ predicting departures from it. The
structure is static rather than dynamic, so the analogy is to the technique rather than to any
particular application of it, such as the expansions used for fast mean-reverting stochastic
volatility. What makes the direction concrete is that the target already exists: the residuals
in Table~\ref{tab:mixture} and their empirical counterparts are what such an expansion would
have to reproduce. We do not attempt it here.
...
Unpublished, cited by Yellott \cite{yellott1977} for the Gumbel representation of the
choice axiom.
