...
structure. The tail asks whether the population departs from the constant-ratio rule at all.

Two rows show the questions coming apart. Wikipedia has a negative gain and a small tail: it
departs from the rule in the direction of contraction and still predicts worse. Sports
participation has a positive gain and a tail of $0.51$: the gain is shrinkage and says nothing
about the rule. Section~\ref{sec:empirical} takes up the first question and
Section~\ref{sec:null} the second.

Each consumer experiment appears twice because a gain and a tail have to come from the same
people, and the forced-choice subgroup is not the whole sample. Three further populations appear
