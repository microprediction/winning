...
random-utility models it is the model with Gumbel noise; Holman and Marley are credited with that
representation \cite{lucesuppes1965}, and Yellott \cite{yellott1977} showed it is not identified from pairwise
comparisons alone but is unique once triples are included. Assume Gaussian noise
...
renormalization is one such model rather than a consequence of probability theory. Within the class of independent
additive random-utility models, Gumbel errors generate Luce choice probabilities, a
representation attributed to Holman and Marley \cite{lucesuppes1965}; Yellott \cite{yellott1977} showed
that it is not unique from pairwise comparisons but that Gumbel is uniquely identified once
triple comparisons are included. Which generative assumption to make is
...
renormalization is recovered exactly. That is the Holman--Marley representation \cite{lucesuppes1965}. The axiom is a boundary of the class.
...
Lotteries, high cost & observed menus & $.035$ & win \\
US state ballots 2024 & observed ballots & $.005$ & win \\
Lotteries, medium cost & observed menus & $.124$ & draw \\
Consumer goods, experiment 2 & observed menus & $.129$ & draw \\
Lotteries, low cost & observed menus & $.667$ & draw \\
Grocery stockouts & observed stockouts & --- & draw \\
\bottomrule
\end{tabular}
\caption{Fourteen wins, five draws, no losses. The upper block reads subset choice off a
complete ranking; the lower block observes it. Tail frequencies are floored at $1/201$ by
two hundred replicates. The grocery cells give no tail because the comparison is made per
store-category: the race is ahead in 29 of 78 cells and its mean gain sits within
$0.0002$ of the null, so neither map is favoured. Forced-choice subgroups are quoted for
the consumer experiments, since no deferral coding enters those.}
...
concentrated where they should be: the smallest ranking collection, the smallest
consumer experiment, the cheapest lottery frame, and the one setting whose alternatives
...
The consumer experiment is not the only place restriction is observed. Four further
populations are scored by the same protocol, and they differ from each other in what does
the removing.
...
to six alternatives, in a cross-section giving at most two disjoint menus per person. Their
three cost frames are disjoint subject pools differing in how many arithmetic operations
are needed to value a lottery, one, three or five. This dataset is adversarial on the
authors' own analysis: they report that random utility cannot explain the population
behaviour while a logit attention model survives.
...
Grocery stockouts supply physical removal. In FreshRetailNet-50K an hourly flag marks a
product out of stock, and shoppers arriving afterwards face a smaller menu that nobody
arranged. The alternatives here are not mutually exclusive, since a shopper may buy two
items in a category, so this is a boundary case rather than a clean test.
...
Grocery stockouts & 78 cells & $-0.0015$ & $-0.0013$ & $-0.0002$ & --- \\
\bottomrule
\end{tabular}
...
are matched state pairs; Wikipedia units are pages whose destination set shrank; grocery
units are store-category cells, scored separately, hence no single tail. Two rows have a
...
Psychology}, volume III. Wiley, 1965.
Yellott \cite{yellott1977} attributes the Gumbel representation of the choice axiom to
Holman and Marley as cited here; it carries no separate entry in Yellott's bibliography.
