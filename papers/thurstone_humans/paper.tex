...
practice and built into almost every layer of every modern neural network: a softmax is
Luce's rule exactly, every attention head applies one, and masking a head renormalizes
over the positions that survive. But a
...
therefore repeat everything on simulated populations that renormalize exactly. Except in
the smallest collection, the observed advantage exceeds that null effect, and in the large
ones the null effect runs the other way.
\end{abstract}
...
relative to renormalization. Captured is the race's share of it. The eleven datasets are
those clearing the null of Table~\ref{tab:null}; sports participation is excluded
throughout. Renormalization captures none of the gap, being the unique map that preserves
every ratio.}
\label{tab:gap}
\end{table}
