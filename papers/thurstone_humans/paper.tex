...
of a softmax, proportional renormalization is the reflex and in many models the architecture. It
is the Gumbel point of the independent additive random-utility class\footnotemark[2]. Re-running
a Gaussian contest among the survivors is Case V. Psychology has tested renormalization under
restriction since 1957, as the constant-ratio rule, and we score the two out of sample on
thirty-nine population comparisons, twenty-seven observing the smaller menu and twelve inducing
subset choice from rankings. Gaussian renormalization has the lower held-out log loss in thirty.
The rows are dependent and unequally clustered across studies, so that count describes this
corpus rather than estimating a rate. The losses concentrate on one-dimensional and strongly
confusable stimulus sets, which motivates a boundary rule rather than establishing one.
\end{abstract}
...
it. The figure asks where each population falls between the two maps, on a scale where linear
...
nats per prediction, so a positive gain means Gaussian renormalization predicted better. The tail
is the Monte Carlo frequency with which a population obeying the constant-ratio rule exactly
reaches that gain on the same menus, and it therefore tests departure from the rule rather than
predictive superiority. Replicates are two hundred except for the news row, sixty, and the Wills
row, four hundred; the tone matrices are participant-averaged, so no tail is computed.}
\label{tab:scoreboard}
\end{table}

\footnotetext[4]{The news tail rests on sixty replicates, so $.016$ is the floor $1/61$ rather
...
direction of contraction. The consumer experiments appear twice each because the two answers
must come from the same analysis population: the forced-choice subgroup admits no deferral
coding, the all-subject figure uses the whole sample, and pairing a gain from one with a tail
from the other would compare different people. Three further populations appear in
Section~\ref{sec:boundary} rather than here, because published aggregates fix the direction of
their failure without supporting this protocol. Wikipedia has a small tail and a negative gain, so it departs from the
...
Hodge \cite{hodge1967} ran nested ensembles in three modalities, Rich \cite{rich1971} carried
...
Thurstonian replacement. They go furthest of anyone toward the contest studied here: alongside
their fitted four-parameter model they construct a ``simple winner-take-all'' system that takes
the noisy magnitudes and picks the largest, note that ``like the ratio rule, the simple-WTA
system has no free parameters'', and report that Gaussian noise ``produces comparable results''
to the rectangular noise they use.

It is worth being exact about what that does and does not settle, because the object differs.
Their magnitudes are supplied by a model of the stimulus, a univariate function of how many
category-appropriate symbols a display contains, and their conclusion is explicitly conditional on
it: the ratio rule is shown incorrect ``given the assumption'' that the magnitudes take that form.
Their comparison is therefore between two theories of how a stimulus produces a choice. The
problem here has no stimulus in it. The input is a probability vector and the output is a
probability vector over a subset of the same support, so an analyst holding shares and nothing
else cannot run their model at all: there are no magnitudes to race. What is common is the
principle, and what is missing there is a map.
...
model of a latent contest and the one Thurstone proposed.

The asymmetry between the two is worth naming. Case V is a model somebody proposed and defended.
Linear renormalization is the Gumbel-noise member of the same class, and it is not adopted
because anyone argues that choice errors are double exponential. It is adopted because dividing
by a smaller total is the obvious thing to do. Nothing in a masked softmax expresses a belief
about extreme-value noise. The incumbent therefore holds its position by default rather than by
argument, and the question below is not whether a challenger can overturn a defended position but
whether the default was ever the right place to stand.

Interpolating between them, or choosing the standardized
...
That assumption should be stated rather than left implicit, because it is the one doing the work.
Both maps hold the latent quantity fixed and change only the menu: the locations that reproduce
the full-menu shares are the locations that run the restricted contest, and the worths that
reproduce them are the worths that get renormalized. Nothing here tests that invariance, and
Section~\ref{sec:boundary} collects the cases where it visibly fails, which is where removal
changes discriminability or the surviving alternatives are not the alternatives that were there
before. Two further assumptions ride along. The noise is independent and identically shaped
across alternatives, which near-substitutes violate. And the population is treated as one chooser
with one latent vector, though Section~\ref{sec:mixture} shows heterogeneous Luce choosers
reproducing aggregate contraction with no individual departing from the axiom. Three
...
its centred excess, and a Monte Carlo tail. The excess is a diagnostic centred on that particular
...
the excess over the null exceeds the raw gain in every remaining collection. It also
...
the ten-label row and predicting the restricted rows, linear linear renormalization wins all four conditions, by $0.0133$ and
...
renormalization and usually better. It has the lower held-out log loss in thirty of the
thirty-nine comparisons, and on the contraction axis of Figure~\ref{fig:line} it is the nearer of
the two maps in eighteen of the nineteen populations that axis can place, the exception resting
on five pairs. Neither statement identifies a population rate, and the second is a projection
rather than a proper score. What they jointly rule out is the position that the two maps trade
blows.
