...
renormalization is the reflex. Nothing licenses it. Conditioning does not apply, because
the experiment has changed rather than information arriving about a fixed one. The
alternative reads the probabilities as winning probabilities in a contest of Gaussian
performances and re-runs the contest over what remains, fitting nothing. It forecasts
better on an experiment where subjects chose from all thirty-one subsets of five goods, and
on twelve of twelve ranking collections. Contraction of a noisy estimate lowers log loss on
its own, so we check against populations that renormalize exactly; the advantage survives
except in the smallest collection.
\end{abstract}
