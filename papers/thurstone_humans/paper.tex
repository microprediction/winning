...
renormalization is one point and Gaussian renormalization another; most fall beyond the Gaussian
point, and the two nearest the linear one are a concentrated-share collection and a perceptual
continuum. The table asks the different question of which map forecasts better. Thirty-nine
...
vocabularies, scratched fields, delisted catalogues and disallowed response sets. Truncation
belongs here in one of its two senses only. If the support shrinks because one has learned that
the outcome lies in $T$, then $p_i / \sum_{j \in T} p_j$ is the conditional probability, linear
renormalization is a theorem rather than a choice, and nothing here bears on it. If it shrinks
because the mechanism has been altered so that only $T$ can occur, as with a decoder confined to
a sub-vocabulary or a field with scratchings, then the old experiment's conditional probability is
not an answer to a question about the new one, and the two maps below are the candidates.
