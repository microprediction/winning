...
Case V. Any noise law fixed in advance gives such a map, and these two are the conventional
choices: the first is the reflex, the second the classical Thurstone model. Psychology has
...
A population chooses among a menu of alternatives and its shares are known. One or more of the
alternatives is then withdrawn: a horse is scratched, a product is delisted, a candidate fails
to qualify for a ballot, a response option is disallowed, a token is masked out of a softmax.
...
Table~\ref{tab:scoreboard} states the result before the argument for it. Thirty-four
populations have full-menu shares that calibrate both maps and restricted-menu choices that
score them by log loss. A comparison is a win for Gaussian renormalization when its gain
exceeds the gain a population obeying the constant-ratio rule would produce on the same menus,
with a Monte Carlo tail frequency of $0.05$ or less, a loss when linear normalization exceeds
it on that test, and a draw otherwise. Section~\ref{sec:empirical} gives the protocol, the
datasets and the null.
...
The losses are not scattered. Four of the five are the same task, absolute identification of
tones, and the fifth is the one sound-identification condition whose survivors are mutual
confusions. The draws fall at the smallest ranking collection, the smallest consumer experiment
and the two gamble frames whose shares are most concentrated. The two largest wins, colour and
symbol naming, come from the one experiment that supplies its own control.
...
Nothing requires standing on exactly one point of it. Anyone with restricted-menu observations
to fit can interpolate, and will do better than either default. The question here is narrower.
...
model of a latent contest and the one Thurstone proposed. Interpolating between them, or fixing
a scale for some third law, requires a number that the shares themselves do not supply. Both are calibrated
...
alternatives a positive covariance. It is not available as a default. The analyst holds $K-1$
free shares, and the model carries $K(K-1)/2 - 1$ free covariance parameters, so for
$K \ge 3$ the shares cannot identify them. A correlated model is available to anyone with
...
Five collections record choices from a smaller menu directly. They are reported first because
nothing about the restriction is inferred.
...
\subsubsection{Probability that the favourite finishes second}

The next decomposition is an error rather than a difference. Both maps can be asked how often
a given item finishes second, which is the restriction question with the winner removed.
...
Choosing the better of these two maps is therefore a smaller improvement than
...
\emph{Gaussian renormalization}, to the status of a tuning-free alternative to the ubiquitous
practice of linear renormalization of probability.

The evidence suggests that, contrary to the overwhelming popularity of the former, the latter
is the more reliable default. Gaussian renormalization wins twenty-four of the thirty-four
comparisons and loses five, and where it wins it clears a null in which the constant-ratio rule
holds by construction.

We have also tried to advance heuristics that caution the reader. Four conditions mark the
cases where linear normalization is the better choice, or where the choice does not matter, and
each can be checked before any restricted-menu observation exists. The alternatives lie on a
perceptual continuum, so that neighbours are more confusable than distant pairs. A withdrawn
option is a near-substitute for a survivor, so its mass returns to that neighbour rather than
spreading. The withdrawal changes how easily the survivors can be told apart, so they are not
the same alternatives before and after. Or one share already holds most of the mass, in which
case the two maps agree to within the noise. The within-set confusion mass of
Table~\ref{tab:getty} is the sharpest of these, and it is computed from full-menu data alone.

To repeat, this note is about competing reflexes rather than competing careful modelling
paradigms. Both
alternatives are nested by the independent additive random-utility family, indexed by its noise
law, and both maps are wrong in the same direction wherever the truth can be measured, by more
than they differ from one another. A fitted noise scale beats both wherever a restricted-menu
observation exists to fit it on.

The same dial recurs outside choice modelling. Masking a softmax and rescaling the survivors is
linear normalization at scale, so any model that restricts its output vocabulary at inference
time has adopted the Gumbel assumption by default.
