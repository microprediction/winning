...
\begin{abstract}
\noindent Given probabilities over mutually exclusive events, and told that one did not
occur, proportional renormalization is the posterior only if the telling says nothing about
which of the others did. Monty Hall is the standard reminder that it usually says
something. Told instead that one has been \emph{withdrawn}, scratched, delisted, masked out
of a softmax, conditioning does not apply at all: the experiment has changed.
Renormalization is what almost everyone does anyway. The alternative reads the
probabilities as winning probabilities in a contest of Gaussian performances and re-runs
the contest over what remains, fitting nothing. It forecasts better on an experiment where
subjects chose from all thirty-one subsets of five goods, and on twelve of twelve ranking
collections. Contraction of a noisy estimate lowers log loss on its own, so we check
against populations that renormalize exactly; the advantage survives except in the smallest
collection.
\end{abstract}
...
automatically Bayes survives repeated exposure to it. Withdrawal is a third case again,
neither flat-likelihood conditioning nor informative conditioning, because no information
about a fixed experiment has arrived: an alternative is gone and the choice problem is a
different one. Nothing licenses renormalization there, and the question of what to do
instead is empirical.
