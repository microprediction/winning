...
Thurstonian replacement, and they come closest of anyone to the exercise below: alongside their
fitted four-parameter model they construct a ``simple winner-take-all'' system that takes the
noisy magnitudes and picks the largest, observe that ``like the ratio rule, the simple-WTA system
has no free parameters'', and report that Gaussian noise ``produces comparable results'' to the
rectangular noise they use. They set it aside because its fit is ``by no means as good'' as the
fitted model's, they judge by the shape of derived functions rather than by held-out score, and
they never compute the point prediction their own equations imply for the restricted share.
