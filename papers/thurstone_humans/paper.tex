...
\begin{abstract}
\noindent A population's full-menu choice shares do not determine its choices from
smaller menus. Luce's axiom supplies one rule connecting them, renormalization, which
leaves the odds between survivors untouched; an independent Gaussian race supplies
another, which makes the survivors less lopsided. We observe that the two are
calibrated from exactly the same shares, so they can be compared as \emph{restriction
maps} with nothing fitted to the smaller menus, and we score them out of sample. The
Gaussian map is better on an experiment in which subjects chose separately from all
thirty-one subsets of five goods, and on twelve of twelve archival ranking
collections. A complication is that the race's prediction is a contraction of the
estimated shares, and contracting a noisy estimate lowers log loss whether or not the
axiom fails; we therefore repeat everything on simulated populations in which the axiom
holds exactly, and the advantage survives. The recommendation is narrow: when mapping aggregate full-menu shares to
restricted-menu shares, make the race the default.
\end{abstract}
