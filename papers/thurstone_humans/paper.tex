...
A mechanism is available, and it comes from the same caveat that limits what the data can
identify.

By Yellott's theorem an individual Luce chooser is a contest with Gumbel noise. Let
individual log-worths vary across the population,
\begin{equation}
\log u_i(\theta) = \mu_i + \varepsilon_i(\theta),
\label{eq:hetero}
\end{equation}
so person $\theta$ chooses $\arg\max_i(\mu_i + \varepsilon_i + G_i)$ with $G$ independent
Gumbel. The population is then itself a contest, with noise the convolution
$\varepsilon + G$. If individual taste is a sum of many small independent influences, the
central limit theorem makes $\varepsilon$ approximately Gaussian. As the scale of
$\varepsilon$ grows relative to the Gumbel scale, the convolution is dominated by its
Gaussian part.
...
\begin{tabular}{rrrr}
\toprule
$\sigma$ & Observed $\lambda$ & Case V $\lambda$ & Ratio \\
\midrule
$0$ & $-0.002$ & $0.227$ & $0.00$ \\
$0.5$ & $0.052$ & $0.228$ & $0.23$ \\
$1$ & $0.127$ & $0.229$ & $0.56$ \\
$2$ & $0.200$ & $0.228$ & $0.88$ \\
$3$ & $0.214$ & $0.227$ & $0.94$ \\
$4$ & $0.221$ & $0.226$ & $0.98$ \\
...
\bottomrule
\end{tabular}
\caption{Simulated populations of Luce choosers whose log-worths carry Gaussian taste
variation of scale $\sigma$, against a Gumbel choice-noise scale of $1$. Observed
$\lambda$ is the population's contraction; Case V $\lambda$ is what an independent
unit-variance Gaussian race implies for that population's own full-menu shares. At
$\sigma = 0$ the population is homogeneous Luce and does not contract, as the axiom
requires. By $\sigma = 4$ the population contracts within two per cent of the Case V
prediction.}
\label{tab:mixture}
\end{table}

So a population of Luce choosers with Gaussian taste variation is Case V in aggregate,
once taste outweighs choice noise. That is a reason to expect the Gaussian transport to
work on population shares which does not require any individual to be non-Lucean, and it
makes the aggregate caveat of Section~\ref{sec:scope} into the mechanism rather than an
apology. It also predicts where the fit should be worst: in populations whose members are
nearly alike.
