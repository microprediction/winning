...
Suppose an alternative's latent performance for one respondent is an additive
accumulation of many small independent influences,
\begin{equation}
X_{ni} = b_{ni} + \sum_{r=1}^{n} w_{nr}\, \varepsilon_{ri},
\qquad \varepsilon_{r1},\dots,\varepsilon_{rK} \ \text{iid Gumbel},
\label{eq:agg}
\end{equation}
so that each individual contribution $r$, taken by itself, is exactly a Luce choice.
Let $s_n^2 = \sigma^2\sum_r w_{nr}^2$. If the weights spread
($\max_r|w_{nr}| / \sqrt{\sum_r w_{nr}^2} \to 0$) and the locations scale with the noise
($b_{ni}/s_n \to a_i$), then the multivariate central limit theorem applies coordinatewise,
the components are independent across alternatives, and the continuous mapping theorem
applies to $\arg\max$ because a Gaussian limit puts no mass on ties. So the winning
probabilities converge to those of the independent unit-variance Gaussian race of
Equation~\eqref{eq:winprob}.

This is folklore rather than a new theorem: it is the CLT plus continuous mapping, applied
to a construction Yellott already identified. Two features of it are worth naming. The
scaling condition is not decoration, since holding $b_{ni}$ fixed while the noise grows
sends every share to $1/K$ and recovers the Gaussian race only at near-indifference. And
the Gumbel plays no role in the limit, only in making each microscopic component exactly
Luce; any finite-variance component law gives the same Gaussian. Thurstone's race is the
additive-aggregation limit, and Luce sits in its basin of attraction.
