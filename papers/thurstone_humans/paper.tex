...
We read the gradient as the mechanism working rather than as instability. When the arithmetic is one
...
Four conditions are worth carrying away, and none is settled by the share vector alone. The
alternatives lie on a perceptual continuum, so that neighbours are more confusable
...
noise scale cannot serve as that parameter, since it is absorbed exactly by the fitted locations.
