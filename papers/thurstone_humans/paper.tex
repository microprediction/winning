...
\begin{abstract}
\noindent Given population shares over a set of alternatives, and asked for the shares
after one or more alternatives are withdrawn, the temptation to renormalize
proportionally is overwhelming. It is ubiquitous in practice and built into standard
softmax mechanisms, where masking and recomputing preserves the relative odds of the
survivors. It is reinforced by a genuine fact: for mutually exclusive events, one of
which is learned to be impossible, proportional renormalization is exactly Bayes.
Withdrawing an alternative from a menu is not that operation. It changes the problem the
chooser faces rather than supplying information about a fixed one, and which transport is
right becomes an empirical question. The alternative studied here reads the shares as
winning probabilities in a contest of independent Gaussian performances, recovers the
locations that imply them, and re-runs the contest over the alternatives that remain. Both
maps use the original shares and nothing else, so they can be scored against each other
out of sample with no fitting on either side. The contest is the better forecast on an
...
contest's prediction is a contraction of the estimated shares, and contracting a noisy
estimate lowers log loss whether or not proportional renormalization is wrong; we
therefore repeat everything on simulated populations that renormalize exactly, and except
in the smallest collection the observed advantage exceeds that null effect.
\end{abstract}
...
Suppose an alternative's latent performance is an additive accumulation of many small
independent influences, each of which is on its own a Luce choice.

\begin{lemma}[Additive aggregation of Luce components]
\label{lem:agg}
For each $n$ let
\begin{equation}
X_{ni} = b_{ni} + \sum_{r=1}^{n} w_{nr}\, \varepsilon_{ri},
\qquad i = 1,\dots,K,
\label{eq:agg}
\end{equation}
where for each $r$ the shocks $\varepsilon_{r1},\dots,\varepsilon_{rK}$ are iid Gumbel with
variance $\sigma^2$, independent across $r$. Put $s_n^2 = \sigma^2 \sum_r w_{nr}^2$ and
suppose
\[
\frac{\max_r |w_{nr}|}{\sqrt{\sum_r w_{nr}^2}} \longrightarrow 0,
\qquad
\frac{b_{ni}}{s_n} \longrightarrow a_i .
\]
Then for every $i$,
\[
P\bigl(X_{ni} = \max_j X_{nj}\bigr) \longrightarrow
P\bigl(a_i + Z_i = \max_j (a_j + Z_j)\bigr),
\qquad Z \sim N(0, I_K).
\]
\end{lemma}

\begin{proof}
Subtracting the common mean $m \sum_r w_{nr}$ from every coordinate and dividing by $s_n$
leaves the $\arg\max$ unchanged, giving
$Y_{ni} = b_{ni}/s_n + Z_{ni}$ with
$Z_{ni} = \sum_r w_{nr}(\varepsilon_{ri}-m) / (\sigma \sqrt{\sum_r w_{nr}^2})$. The Gumbel has
moments of every order, so the weight condition is Lindeberg's and $Z_{ni} \Rightarrow
N(0,1)$; the shocks are independent across $i$, so
$(Z_{n1},\dots,Z_{nK}) \Rightarrow N(0,I_K)$, and $b_{ni}/s_n \to a_i$ with Slutsky gives
$Y_n \Rightarrow a + Z$. The map $\arg\max$ is discontinuous only at ties, which the
Gaussian limit assigns probability zero, so the continuous mapping theorem transfers the
convergence to the winning probabilities, the outcome space being finite.
\end{proof}

The right-hand side is Equation~\eqref{eq:winprob}. The lemma is elementary, being the
central limit theorem and continuous mapping applied to a construction Yellott already
identified, and we state it because we have not found it stated. Two of its features are
worth naming. The scaling condition is not decoration: holding $b_{ni}$ fixed while the
noise grows sends every share to $1/K$ and recovers the Gaussian race only at
near-indifference. And the Gumbel plays no part in the limit, only in making each
microscopic component exactly Luce; any finite-variance component law gives the same
Gaussian. Thurstone's race is the additive-aggregation limit, and Luce sits in its basin of
attraction.
...
One caveat about what has been aggregated. Equation~\eqref{eq:agg} accumulates latent
components within a respondent, not choosers within a population; calling it a theorem
about aggregating Luce choosers would overstate it. The simulation above does the latter
and reaches the same limit, but a mechanism that makes the additive accumulation natural
rather than assumed is left open.
