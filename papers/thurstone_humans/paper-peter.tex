...
\begin{abstract}
\noindent 
Re-scaling probability vectors after a first choice is assumed or an item is withdrawn, delisted or 
Masked out is one of the great unconscious modeling acts. But it is equivalent to assuming a Gumbel distribution
In the class of additive random-utility class of models, whereas Gaussian is more natural for a variety of reasons. 
We call the latter ``gaussian renormalization'' and set it alongside linear (proportional) renormalization in a 
Two horse empirical race, which is reasonable because neither has any tuning parameter. In part we re-litigate the 
Psychology debate that compares the constant ratio rule (Luce' Choice Axiom) against Thurstone Class V Normal models, but
We also introduce new data from a variety of situations where renormalization of probability is tempting. We find
That Gaussian renormalization has lower held-out log loss in thirty, eighteen of the twenty-seven observed restrictions and twelve of the twelve induced from rankings.
\end{abstract}
...
Suppose a probability vector is presented, as with a collection of de-vigged bookmaker odds for a horse race or
A calibrated probabilistic machine learning model. Suppose next that the set changes. Perhaps a 
Horse is scratched, a product is delisted, or a primary candidate drops out of a race. Alternatively a response
Option in a survey maybe disallowed or a token might be masked out of a softmax. These are quite 
Different situations with their own nuances but in each case a probability vector
 has to be transported from the original
support to a smaller one. The natural question arises: what are the probabilities over what remains?

A related problem also arises in the computation of permutation probabilities but here we will focus on the 
Case where the original vector is a market share of sorts - as with population popularity of an item on a menu. This
Data is the more controversial in some respects because it can speak to discovery or delineation of underlying mechanics from
Revealed preferences. In contrast, it is somewhat self-evident that the correct answer for modeling contest winning probabilities
Can often be strongly suggested by the generative model at hand (for example, finishing time distributions). 


When we remove a menu item, quite a lot of machinery exists for estimating carefully the probabilities that should be ascribed to 
The new reduced menu. It goes without saying that depending on the data available to the problem, probit models or other tools can be brought
To bear. That, however, is outside the scope of the present discussion because wielding statistical machinery together with 
Careful modeling, parameter selection, cross-validation and so forth can evolve into an entire enterprise and it is often unreasonable
To compare this to the trivial, instinctive act of rescaling probability. That reflex cannot be replaced or challenged by an invitation to 
Treat every occasion for its use as a new data science project. 

There is one alternative response that is slightly more analyticaly involved but not methodologically complicated. It is the Gaussian special case
Of the Thurstone Class V family and we will argue for its natural place as a tuning-free default renormalization to challenge linear. Both are on
Equal footing philosophically. Nothing has been observed about the smaller menu, so the answer must come from an assumption
...
We suggest that gaussian renormalization is equally natural, if not more so. Read the shares as the winning probabilities of a contest among
independent Gaussian performances (which as noted has heritage back to 
...
Our limited goal is a practical comparison of linear renormalization and Gaussian renormalization.
The first holds the ratios of the shares fixed, the second holds the latent locations fixed. The underlying
Physics is quite different and so even small persistent statistical advantages of one over the other may be 
Suggestive of structure and interesting to the psychology community but also, we hope, the broader machine learning audience whose
Choice of linear renormalization is often tacit. 


Each competitor in this two horse race is elementary and not surprisingly has assumed many names. We treat them as synonyms. We equate
proportional renormalization, linear renormalization, the ratio rule, Clarke's constant-ratio
rule, masking a softmax and rescaling the survivors. All denote Equation~\eqref{eq:luce}
applied to a subset, and we write linear renormalization except where the historical name is
what a cited paper used. 

Conversely we equate gaussian renormalization, Thurstone's Case V re-run
...
comparison we present had not been always been made broadly or crisply is that the two literatures holding the pieces did not share
a vocabulary.
...
counts the favourite's advantage for less. It is remarked in Section~\ref{sec:maps} that these two are the
two conventional members of the independent additive random-utility class. That is well appreciated. 
Proposition~\ref{prop:contraction} makes the direction of their disagreement exact.

\section{Scoreboard}

Lest we be accused of burying the lead, Figure~\ref{fig:line} and Table~\ref{tab:scoreboard} are intended to sway the reader
And provide a jolt to anyone whose strong inclination is Luce's Choice Axiom, especially the few who might have
Been under the impression that dividing probabilities constitutes an argument of itself! 

The figure asks where each population falls between the two kinds of renormalization, on a scale where linear
...
The single population nearer linear renormalization comes from one experiment where people were asked to 
Rank gambles they would enter into (which takes on a bit of the flavor of a quiz and so might be classified separately). The figure
Is highly suggestive but also slightly dishonest in some respects since the statistic is not a proper scoring rule. 


The table is more statistically defensible in that last sense but also compelling as a refutation of the notion that linear 
Renormalization has any natural advantage or should occupy the special place that, in practice, it tends to claim. 
Thirty-nine comparisons have full-menu shares that calibrate both maps and restricted-menu choices. The score
Is held-out log loss. 

Another caveat: rows are comparisons. Several
reuse the same subjects or source study so they are not to be considered independent tests of the same hypothesis. Section~\ref{sec:empirical} goes into complete
Detail on the protocol, the datasets and the null hypothesis. 
...
lie beyond $r = 1$: choice contracts by more than Case V predicts, so as a general observation, the Gaussian point tends not
far enough along the family. We hasten to add that this statistic is not proper, however, so refer the reader also to table \ref{tab:scoreboard}.  }
\label{fig:line}
\end{figure}
...
nats per prediction. Here we read a positive gain as a win for Gaussian renormalization. Replicates are two hundred except
 for the news row and the Wills
row.}
...
Gaussian renormalization performs strongly. It has the lower held-out log loss in thirty of the thirty-nine rows.
That count is a description of this table and not an estimate of any rate, because the rows are
not independent, the families contribute unequal numbers of them.\footnote{As a finer point,the collections were
assembled by searching for restriction data. It could be argued that this is therefore not a fair sample from a population of
applications.} 

We forward an explanation for eight of the nine rows where linear renormalization seems to win. 
Four are tone-identification and three are line-identification. Another is the Getty
condition whose four survivors are mutual confusions. The common thread is that all place the alternatives on a
...
Thurstonian replacement, and they go furthest toward the contest studied here: alongside their
fitted four-parameter model they build a ``simple winner-take-all'' system that picks the largest
noisy magnitude, note that ``like the ratio rule, the simple-WTA system has no free parameters'',
and report that Gaussian noise ``produces comparable results''.

The principle is therefore theirs. The map is not, and the difference is in what goes in. Their
magnitudes are supplied by a model of the stimulus, a function of how many category-appropriate
symbols a display contains, and their conclusion is conditional on it: the ratio rule is incorrect
``given the assumption'' that magnitudes take that form. The problem here has no stimulus in it.
The input is a probability vector, not a probability vector together with a picture of whatever
produced it, so an analyst holding shares alone has nothing to race. Their Experiment 2
nonetheless withdraws an alternative, and their data has been deposited throughout, so it enters
the comparison below on the same terms as everything else.
...
the rest obtained. Monty Hall is a famous counterexample. Withdrawal is a third case,
...
\subsection{What the failures do not show}
...
\subsection{What they do show}
\label{sec:diagnostics}

So the pattern is not a semantic rule about what kind of thing the alternatives are. It is a set
of diagnostics for assumptions, and the three are of different logical types.
...
Section~\ref{sec:diagnostics} exhibits a one-dimensional continuum on which Gaussian
