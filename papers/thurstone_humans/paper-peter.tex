...
Yeon and Rahnev & four colours, menu revealed after & $+.0147$ & $[+.0118, +.0179]$ & Gaussian \\
...
\midrule
\multicolumn{5}{l}{\emph{Ecological: the menu changed for reasons outside the design}} \\
Microsoft News & 6{,}000 nested slate pairs & $+.0496$ & not computed here & unresolved \\
US state ballots 2024 & 263 matched state pairs & $+.0002$ & $[+.0001, +.0002]$ & Gaussian\footnotemark[11] \\
Wikipedia clickstream & 3{,}544 pages & $-.0001$ & not computed here & draw \\
\midrule
...
honest unit because the rows cluster, it is ahead in fourteen of twenty with one going the other
...
the original and \texttt{python derive\_rouder.py} rebuilds the counts from it, reproducing every
figure reported here exactly, and \texttt{MANIFEST.tsv} records the upstream git blob hash of
every file the reduction was built from.
