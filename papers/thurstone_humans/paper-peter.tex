...
So the race's advantage is not a property of these pairs, these colours or
...
The aggregate runs over every subset of size two or more, which includes $T = S$. There the
two maps coincide and the gain is zero by construction, so the full menu contributes a
guaranteed tie. It is one subset in $1{,}013$ at $K = 10$ and one in four at $K = 3$, so it
dilutes small collections much more than large ones. Restricting the estimand to
$2 \le |T| < K$ raises every gain and never lowers one: by a factor of $1.33$ on Netflix, $1.10$
on the three $K = 4$ collections, $1.04$ on the two General Social Survey items and $1.00$ to
two decimal places on everything with $K \ge 7$. Political goals moves from $+0.0069$ to
$+0.0076$ and Netflix from $+0.0009$ to $+0.0013$; the rest move in the fourth decimal or not at
all. The figures quoted throughout include the tie and are therefore the conservative ones.
...
removed. Nine collections fall monotonically in $|T|$. Occupational prestige and sports
...
The third is the score itself. Log loss is unbounded below, and Table~\ref{tab:byrank} shows
...
tried alongside linear renormalization rather than passed over. Four questions bear on whether the
independence the Gaussian contest assumes is tenable. None is settled by the share vector alone,
and how well they can be answered in advance depends on what else is available. Distinct unordered
items, such as goods, articles, jokes or occupations, usually answer no to all four, which is why
they are where the two maps separate most cleanly, but that is a rough guide to the four questions
...
contest's own prediction, and the two come apart on individual pairs and triples.
...
ones. A masked softmax is the arithmetic of the second case and the question of the third,
...
nearer of the two maps in eighteen of the nineteen populations that axis can place. Neither count
...
What the boundary conditions of Section~\ref{sec:boundary} amount to is not established. They are
diagnostics rather than a rule: dependence among the alternatives, a task that changes when an
...
judgement can be wrong. Gaussian renormalization is a credible tuning-free alternative to linear
renormalization, and this note compares two reflexes rather than two careful modelling exercises.
