...
output layer obeys Luce's Choice Axiom for that operation. Telling a model that
an option is unavailable is a different operation, and we measure what law it
follows. The axiom fails its sharpest test: restricting a choice to two named
items shifts their log odds by $-1.15$ nats $[-1.547, -0.808]$, where the axiom
requires zero. The failure is not a rescaling. A third of surviving pairs
\emph{reverse} rank under restriction and the leading survivor changes in half of
cells, so no rank-preserving rule can be right. Among rules that are wrong, a
Gaussian contest is wrong by less than renormalization in all 99 question types
of a $37{,}764$-cell sweep, and calls the direction of the odds failure in
$71\%$ of pairs, with the median pair matching its predicted magnitude. Explicit menus violate every random utility model.
\end{abstract}
