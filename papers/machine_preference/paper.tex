...
entirely, gives $0.39$ $[0.308, 0.466]$ for people over five domains and $0.69$
$[0.585, 0.792]$ for machines over 50 categories. A Thurstonian contest
calibrated to each population predicts $0.298$ and $0.120$ respectively, so
people are close to the contest and far from the axiom, while machines overshoot
the contest almost six-fold. Machine choice also fails in a way no random utility
...
both families at once, and each population can be compared with the contest
calibrated to its own unrestricted distribution. Case V predicts a larger
discount for people than for machines, $0.298$ against $0.120$, because human
distributions are flatter and their calibrated locations correspondingly closer
together; a shorter field compresses less.

For people the classical dispute has a clear answer on this statistic. The
contest predicts $0.298$, the observation is $0.389$, and the intervals overlap,
while the axiom's zero lies far outside them. A century of argument about whether
choice under restriction renormalizes or re-races comes down, in these five
domains, on the side of the race.

For machines it has no answer, because neither family is close. The contest
predicts $0.120$ and the observation is $0.690$, an overshoot of $5.8$ with
intervals nowhere near each other. Machines depart from the axiom in the
direction the contest specifies and then continue well past the contest, which
together with the rank reversals of Section~\ref{sec:order} places their
behaviour outside the random utility class rather than at some point within it.

The comparison is therefore not that machines are more Thurstonian than people.
It is that people are close to Thurstonian and machines are not close to
anything: they exaggerate the human departure from Luce by roughly a factor of
two, and the Thurstonian departure by roughly a factor of six.

Three limits bound this. Two of the five human collections are perceptual
discrimination tasks rather than preferences, and pooling them with sushi and
Netflix mixes constructs. Human subset choices are derived from complete
rankings rather than elicited, so the human side cannot exhibit reversals and its
$\lambda$ is measured under a construction that presupposes a stable order.
And the machine and human items are different, so the two $\lambda$ values are
comparable as behaviour under a common scale-free statistic, not as answers to
the same question about the same alternatives.
