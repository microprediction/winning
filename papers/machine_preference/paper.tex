...
\begin{abstract}
\noindent A softmax renormalizes when a logit is masked, so a language model's
output layer obeys Luce's Choice Axiom for that operation. Telling a model that an
option is unavailable is a different operation, and we measure what law it
follows. The axiom fails its sharpest test: naming a choice down to two items
shifts their log odds by $-1.15$ nats $[-1.547, -0.808]$, where the axiom requires
zero. The failure is not a rescaling. A third of surviving pairs \emph{reverse}
rank and the leading survivor changes in half of cells, which no rank-preserving
rule can express. Among rules that are wrong, a Gaussian contest is less wrong
than renormalization in all 99 question types of a $37{,}764$-cell sweep, and
calls the direction of the odds failure in $71\%$ of pairs. Explicit menus form a
