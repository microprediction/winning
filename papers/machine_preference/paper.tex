...
to modern models with exact log-probability designs: replaying the
original two-slot stimuli across 2{,}192 cells, Thurstone beats Luce
decisively (mean $\Delta$KL $+0.55$, CI $[+0.50, +0.59]$), and across
692 permutation-controlled deletion cells ($+0.025$), with the advantage
...
adjective-varied prompt pairs, every adjective included---in its
original two-slot form (``My two favourite human organs are the
[ANSWER] and the [MASK] because they are SOMETHING''), on GPT models
with exact probabilities. Across 2{,}192 cells the conditional second
choice decisively favors the contest model: mean $\Delta$KL $+0.55$,
95\% CI $[+0.50, +0.59]$, on every model (from $+0.36$ on GPT-4o-mini
to $+0.86$ on GPT-4.1) and in both entropy strata. Predicting the
