...
entirely, gives $\lambda = 0.39$ $[0.308, 0.466]$ for people over five domains
and $0.69$ $[0.585, 0.792]$ for machines over 50 categories: people keep about
three fifths of their prior odds, machines about three tenths. Machine choice
also fails in a way no random utility model permits, reversing a third of
surviving pairs.
\end{abstract}
...
\midrule
People (5 domains, PrefLib) & 90 & $0.87$ & $0.389$ $[0.308, 0.466]$ \\
Machines (50 categories, 5 models) & 276 & $1.50$ & $0.690$ $[0.585, 0.792]$ \\
\midrule
Choice Axiom & & & $0$ \\
...
pair. Neither party is near the axiom. People retain about three fifths of their
prior log odds, machines about three tenths, and the intervals do not overlap.}
\label{tab:lambda}
\end{table}

Controlled this way the machine exaggeration is $1.77$ rather than six, and it
survives: the intervals are disjoint. Within bands of starting odds the ordering
is the same at every level, with mean shifts of $-0.04$ against $-0.02$ below
half a nat, $-0.26$ against $-0.07$ from half to one, $-1.07$ against $-0.28$
from one to two, and $-2.36$ against $-0.65$ above two.

So the honest comparative claim is narrower than the raw numbers suggest and more
interesting than either party alone. The Choice Axiom is not a description of
human choice under restriction, which was already suspected and is here
quantified; it is a worse description of machine choice, by a factor closer to
two than to six; and the failure has the sign the contest predicts in both.
