...
\title{\textbf{A Paradox in Machine Preference}\\[0.4em]
\large softmax machines do not obey Luce's Choice Axiom}
\author{Peter Cotton}
...
\cite{cekinmez2026}, 30 independent sentence generations per word, senses
...
collapse replicates almost exactly: mean normalized sense entropy $0.265$
generated (their text-model figure: $0.25$) against $0.906$ stated. Eight
of fifteen words collapsed to a single sense in all thirty samples.

Fitting the two one-parameter families from stated to generated
distributions gives a statistical tie (Thurstone $\sigma = 0.49$, RMSE
$0.293$; power tilt $\gamma = 2.0$, RMSE $0.294$; Thurstone the better
...
For the five words whose unrestricted distributions were non-degenerate,
we re-sampled with the modal sense excluded by instruction and compared
zero-parameter predictions of the restricted distribution: Luce
renormalization against Thurstonian contestant removal, both calibrated
to the add-half smoothed unrestricted sense counts.
...
\midrule
bolt  & 0.449 & \textbf{0.423} & fastener .76, run .17, lock .07 \\
seal  & \textbf{0.340} & 0.347 & close .89, stamp .11 \\
pitch & 0.626 & \textbf{0.580} & throw .06, tone .29, field .65 \\
bat   & 0.750 & \textbf{0.709} & animal 1.00, sports .00 \\
iron  & \textbf{0.076} & 0.130 & metal .93, press .07, golf .00 \\
\midrule
pooled & 0.493 & \textbf{0.468} & \\
\bottomrule
\end{tabular}
\caption{Restriction test on polysemous words (Claude Haiku 4.5, 30
samples per condition, modal sense excluded by instruction). The
Thurstonian prediction is closer pooled and on three of five words.}
\label{tab:restrict}
\end{table}
...
sense (seal $\to$ \emph{close} at $.89$; iron $\to$ \emph{metal} at
$.93$). Remove the winning horse and the runner-up now wins nearly
always---low-noise contest behavior, and the same low-entropy,
heavy-qualification regime where the Thurstonian advantage concentrated in
Section~\ref{sec:maskedresults}. One word (``bat'') flipped rank entirely
under restriction, likely because excluding the \emph{hit} sense also
suppresses the overlapping \emph{sports} sense; inventories with
overlapping senses contaminate the test and neither family survives them.
