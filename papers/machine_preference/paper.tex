...
The objection to such a layer is cost, and it is worth stating what the cost
actually is. Winning probabilities in the normal contest have no closed form,
but they do reduce to a single shared one-dimensional integral,
\begin{equation}
P(X_i \text{ minimal}) = \int \phi(x - a_i)
  \prod_{j \neq i} \bigl(1 - \Phi(x - a_j)\bigr) \, dx,
\label{eq:contest}
\end{equation}
which is differentiable in $\{a_i\}$ and computable for all $i$ at once on a
common quadrature grid. The whole vector costs $O(nG)$ for $n$ items and $G$
grid points against the softmax's $O(n)$, so at the grid resolution of
\cite{cotton2021} a full-vocabulary contest layer is some three orders of
magnitude more expensive per position. That is prohibitive as a drop-in
replacement and unnecessary as a design: a softmax can shortlist the top
hundred candidates and the contest can arbitrate among those, which is where
essentially all of the probability mass and all of the measured disagreement
between the two families lives. We have not trained such a layer, and offer
Equation~\eqref{eq:contest} only to show that the obstacle is a constant
factor on a shortlist rather than an absence of gradients.
