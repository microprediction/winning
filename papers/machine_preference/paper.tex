...
\begin{abstract}
\noindent A softmax output layer renormalizes when a logit is masked, so it
obeys Luce's Choice Axiom for that operation. Restricting a choice set in
language is a different operation: the prompt changes and the logits are
recomputed. We measure which law describes the recomputation. Across thirteen
designs and 48{,}536 cells, scored by exact log probabilities on every design
but one, Thurstonian contestant removal predicts restricted machine preference
better than renormalization does, in all 99 question types of the original
appendix and on all twenty-one models. Most of the reallocation is a
flattening that either family captures given one scale parameter; the contest
geometry earns a further small increment. Explicit menus form a separate
regime that violates every random utility model.
\end{abstract}
...
Winning a two-model contest against renormalization does not establish that a
Gaussian race generates the data. The natural rival is one fitted parameter of
sharpening,
\begin{equation}
...
That comparison is not like for like, since the tilt carries a parameter and
contestant removal carries none, but it locates the phenomenon. The fitted
$\gamma$ is well below one, so restricted distributions are markedly
\emph{flatter} than the unrestricted ranking implies, and both zero-parameter
forecasts are too concentrated. Most of the reallocation is a change of scale.

Giving the contest the matching freedom, performances $N(a_i, \sigma^2)$ with
one fitted $\sigma$, and scoring again on held-out question types, yields
$\sigma \approx 3.4$ and a held-out mean KL of $0.794$. The contest retains a
small advantage over the tilt at equal parameter cost, $+0.0089$
$[+0.0040, +0.0142]$ clustered by question type, favouring Thurstone in 60 of
99 types.

The defensible reading follows. Prompted restriction reallocates probability
rather than renormalizing it, and the dominant component of that reallocation
is a flattening that either family captures once allowed a scale parameter.
Conditional on the flattening, the Gaussian geometry earns a further small and
consistent improvement. What the batteries establish is that machine
restriction behaviour is more Thurstonian than Lucean among the forecasts
considered, not that a stable Gaussian contest runs inside the model. Several
untested alternatives would produce similar reallocation, among them
heteroscedastic or correlated races, heavy-tailed contests, similarity-
sensitive nesting, and retrieval dynamics specific to list completion.
