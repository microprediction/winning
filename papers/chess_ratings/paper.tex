...
offsets. With both systems tuned on the same validation split, the
pooled rating is better by $0.008$ to $0.012$ nats of held-out log loss
across three independent splits, every interval excluding zero; the
estimator accounts for $-0.0080$ of that and the factor structure for
$-0.0033$, and two thirds of the estimator contribution is the
privilege of a batch fit rather than a better method. Once Glicko-2's
recency parameter is tuned rather than left at its default,
stratification is worth nothing to either side. Opening family is unidentifiable because players
...
every arm sees the same games. Both systems have their material
parameters tuned on the validation split: Glicko-2's rating period and
initial deviation, our level and offset ridges. Glicko-2's $\tau$ is
not one of them, because it is inert here: it enters only the
volatility iteration, and sweeping it from $0.02$ to $1.0$ returns
bit-identical loss, so a comparison that tunes $\tau$ and stops leaves
the competitor at library defaults while its own side has a parameter
that bites. Held-out mean log loss is reported on the
...
Glicko-2 per time control \emph{(Lichess)} & 0.6177 & 0.6191 & 0.6232 \\
our scalar & 0.6117 & 0.6134 & 0.6140 \\
\textbf{our factor} & \textbf{0.6073} & \textbf{0.6111} & \textbf{0.6108} \\
\bottomrule
\end{tabular}
\end{center}
...
\midrule
\textbf{factor vs Lichess} & $-0.0104$ & $-0.0080$ & $-0.0124$ \\
estimator (scalar vs Glicko-2 pooled) & $-0.0076$ & $-0.0072$ & $-0.0093$ \\
...
The headline should not be quoted without the decomposition. The
estimator contributes $-0.0080$, a batch fit that sees the training
games at once against an online update that sees each game once, and
the factor structure $-0.0033$. The structure's own contribution, with
intervals excluding zero on all three splits, is the part that answers
the question posed, and it is the column insensitive to how well the
competitor is tuned: tuning Glicko-2's rating period moves the
estimator column by three fifths and the structure column by a
twentieth. The estimator column is also partly a
privilege rather than a better method, because a production system must
process each game once and ours need not, and two thirds of it is that
privilege.

One caveat bounds the margin from the wrong side. Glicko-2's
rating period selects the largest value offered on every arm and every
split, so the margin reported here is an upper bound on the true one.
The parameter saturates rather than running away, moving validation
loss by $0.6203$, $0.6200$, $0.6199$ across three decades, because past
a point it switches deviation inflation off entirely; the residual is
therefore bounded near $0.0001$ rather than unknown.

\section{Stratification buys neither side anything}

Stratifying Glicko-2 appears to gain $0.0063$ to $0.0089$ while
stratifying our scalar loses $0.0006$, and the contrast invites an
argument about which base estimator benefits. The gain is an artifact
of leaving Glicko-2's rating period at its default. With the period
tuned, stratification is worth $-0.0016$, $-0.0015$ and
$-0.0001$ on the three splits, which is nothing, and partial pooling
of the same information still gains $0.0023$ to $0.0044$.

The reading is simpler than the contrast suggests. Per-category
ratings were not buying Glicko-2 category information; they were
protecting it from deviations that inflated too fast for a dense
single-month window, and a tuned period removes the need for that
protection. Conditioning on the category earns nothing once the base
estimator's recency is set correctly, while partial pooling of the same
information still pays. A practitioner who finds that stratification
helps should check the recency setting of the system it helped.
...
is worth $-0.0016$ of the estimator gap, and a colour-blind fit still
beats colour-blind Glicko-2 like for like. Both figures come from runs
against Glicko-2 at its default rating period and are not recomputed
here; the share attributable to colour is small either way, and the gap
it is a share of is $-0.0080$.
...
from $30.9$ to $15.3$ and the margin is flat throughout, every interval
excluding zero. Those margins also come from runs against Glicko-2 at
its default period and will shrink by roughly the headline's
proportion when recomputed, since they share its baseline; the
robustness
question concerns their flatness across thresholds, which does not
depend on the level. The registered prediction, however, was that
...
\paragraph{Two thirds of the estimator column is batch privilege.} Our
estimator revisits the training set and Glicko-2 processes each game
once, because a production system must. Run prequentially over the
month, with both systems tuned on recency and initial uncertainty and
the grids extended until the selection is interior, our tracker beats
Glicko-2 pooled by $-0.0029$ $[-0.0043, -0.0014]$ and the deployed
per-time-control architecture by $-0.0023$ $[-0.0046, -0.0001]$.
Against the batch estimator column of $-0.0080$, that puts $-0.0051$
down to revisiting the training set and leaves $-0.0029$ as a better
estimator under the deployment-realistic protocol, about the same size
as the factor structure. Stratification is worth nothing to either
system there either, $-0.0005$ to Glicko-2 and $+0.0015$ to ours,
reproducing the batch finding on a different protocol.
...
on its own data, by $0.008$ to $0.012$ nats once both systems are
tuned. The estimator contributes $-0.0080$ and the pooling $-0.0033$,
and both parts are worth stating separately. Two thirds of the
estimator contribution is the privilege of revisiting the training set,
and the third that survives a prequential comparison is about the size
of the pooling. The wider claim the data
supports is narrower than ``ratings should be vectors'': it is that
\emph{conditioning} earns nothing once a base estimator's recency is
set correctly, that \emph{pooling} the same information still pays,
and that the covariate must vary within a player for reasons the player
does not choose. Opening choice fails that
