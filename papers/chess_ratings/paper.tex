...
$-0.0033$. Once Glicko-2's recency parameter is tuned rather than left
at its default, stratification is worth nothing to either side. Opening family is unidentifiable because players
...
initial deviation, our level and offset ridges. An earlier version of
this comparison tuned Glicko-2's $\tau$ instead, which is inert in this
implementation and returns bit-identical loss from $0.02$ to $1.0$, so
the competitor ran at library defaults while our side had a working
knob; the numbers below are the corrected ones. Held-out mean log loss is reported on the
...
the question posed, and it is the part that did not move when the
tuning was corrected: the estimator column fell by three fifths and the
structure column by a twentieth. The estimator column is also partly a
privilege rather than a better method, because a production system must
process each game once and ours need not.

One caveat bounds the corrected margin from the wrong side. Glicko-2's
...
An earlier version of this comparison reported that stratifying
Glicko-2 gains $0.0063$ to $0.0089$ while stratifying our scalar loses
$0.0006$, and built an argument on the contrast. That gain was an
artifact of leaving Glicko-2's rating period at its default. With the
period tuned, stratification is worth $-0.0016$, $-0.0015$ and
$-0.0001$ on the three splits, which is nothing, and partial pooling
of the same information still gains $0.0023$ to $0.0044$.

The corrected reading is simpler than the one it replaces. Per-category
...
beats colour-blind Glicko-2 like for like. Those two figures were
measured against the untuned Glicko-2 and are not recomputed here; the
share attributable to colour is small either way, but the absolute gap
it is a share of is now $-0.0080$ rather than $-0.0206$.
...
excluding zero. Those margins were measured against the untuned
Glicko-2 and will shrink by roughly the same proportion as the
headline when recomputed, since they share its baseline; the robustness
...
and both parts are worth stating separately. The wider claim the data
