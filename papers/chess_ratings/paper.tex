...
control. That is stratification: condition on the category, estimate
within it, and accept the sparse-cell variance that follows. We compare
it against a partially pooled alternative on 59{,}399 decisive games
from one month of the site's open database, in which a player's ability
is a vector---one level estimated from all of their games, plus shrunk
bullet and blitz offsets---and find the pooled rating better by
$0.0137$ to $0.0182$ nats of held-out log loss across three independent
splits, every interval excluding zero. The margin decomposes, and the
decomposition matters more than the headline: roughly four fifths is
the estimator, a batch Thurstonian fit against an online update, and
one fifth is the factor structure itself. The same data shows why
stratification is the weaker choice for whoever already has a good
estimator: it \emph{helps} Glicko-2 by $0.0063$--$0.0089$ and
\emph{hurts} our scalar by $0.0006$, because its benefit is capped by
what pooling already captures while its cost, sparse-cell variance,
is not. We also report the covariates that failed. Opening family is
unidentifiable because players choose openings; opponent-strength
curvature is real but almost entirely shared, worth one global
parameter rather than one per player; and the result cannot currently
be replicated on modern Lichess, where only $0.3\%$ of players play
more than one time control against $52\%$ in 2013.
\end{abstract}
...
varies by context---a player strong at speed chess and ordinary at
...
This note asks which is better, against the deployed system rather
than a textbook baseline, and reports what the difference is actually
made of.

\section{Data and protocol}

One month of the Lichess open database (2013-01, CC0), reduced to
headers. We keep decisive games---draws are dropped, a limitation we
return to---between players with at least 100 games in the month,
giving 59{,}399 games among 639 players. Each game is labelled with its
time control.
...
Four arms. \textbf{Glicko-2 pooled} ignores time control.
\textbf{Glicko-2 per time control} is Lichess's deployed
architecture. \textbf{Our scalar} is a batch Thurstonian
maximum-a-posteriori fit with one parameter per player, and exists to
separate the estimator from the structure. \textbf{Our factor} adds
the shrunk offsets.
...
\textbf{The headline should not be quoted without the
decomposition.} Roughly four fifths of the margin is the estimator---a
batch fit that sees the training games at once against an online update
that sees each game once---and one fifth is the factor structure. The
structure's own contribution, $0.003$ to $0.004$ with intervals
excluding zero on all three splits, is the part that speaks to the
question this note asks. The estimator column is a real advantage but
a different claim, and it is partly a privilege: a production system
must process games once, and ours need not.

\section{Why stratification helps them and not us}
...
\textbf{Opening family fails, and the mechanism generalises.} A fair
fit---offset ridge swept to the scalar limit---collapses exactly onto
the scalar. The axis is nonetheless real: among 790 players with at
...
\textbf{Opponent strength is real but shared.} The signed
...
Fitted against a deliberately fair baseline---scalar plus one
\emph{global} curvature coefficient, rather than a bare scalar---the
per-player offsets are identifiable, with a tuned ridge interior at
...
So of three identifiable covariates---level, time control, opponent
strength---the factor treatment pays on one.

\section{Robustness}

\textbf{Colour is not the confound.} Our arms model the white
...
\textbf{The result is not an artifact of the dense subpopulation, but
...
The honest statement is that the factor advantage is roughly constant
in population density rather than concentrated among data-poor players,
which leaves the pooling account unrefuted but unconfirmed by the one
test that could have confirmed it.

\textbf{Our arm is not winning by being overconfident.} Glicko-2
...
\textbf{Batch against online.} Our estimator revisits the training set;
...
\textbf{Draws are dropped}, 3.3\% of games in this pool and far more at
longer time controls and higher strength. A three-way outcome model
would be the honest treatment.

\textbf{One month of 2013, and this is harder to close than it looks.}
...
\emph{falls} as activity rises---the most active players are the most
...
for reasons the player does not choose---a condition that opening
choice fails, that opponent strength satisfies while carrying almost no
per-player signal, and that time control satisfied in 2013 and no
longer does.
