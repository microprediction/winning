...
| [`chess_ratings`](chess_ratings) | Beating a Deployed Rating System: Partial Pooling Against Per-Category Glicko-2 on Lichess | **Note** (2026-09-13, [`STATUS.md`](chess_ratings/STATUS.md)). With both systems tuned, a partially pooled factor rating beats Lichess's deployed per-time-control Glicko-2 by 0.008–0.012 nats over three splits on 59,399 games; the estimator contributes −0.0080 and the factor structure −0.0033. Reports the failed covariates (opening family unidentifiable by self-selection; opponent-strength curvature real but almost entirely shared) and that the modern site can no longer pose the question (0.3% of players mix time controls against 52% in 2013). Experiments in `research/chess/`. |
