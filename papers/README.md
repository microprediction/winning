...
|---|---|---|
| [`factor-probit-transform`](factor-probit-transform) | Scalable Share Calibration for Factor Multinomial Probit Models | **Submitted.** JCGS target (`VENUES.md`); SSRN preprint. Submission build is `scalable-share-calibration-jcgs.pdf` (double-spaced). |
| [`../docs/latex_src/general_inversion`](../docs/latex_src/general_inversion) | Scalable Inversion of Contests with Correlated Performances, Including Softmax and Multinomial Probit | **No venue chosen.** Most active manuscript (nine review rounds actioned); claim-to-script manifest in `CLAIMS.md`; tables pinned at tag `paper-r1`. |
