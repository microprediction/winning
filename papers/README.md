...
| [`passk_posterior`](passk_posterior) | A Posterior Predictive for Pass@k | **Note** (2026-09-02). Measures the plug-in independence extrapolation inside fine-tuning diagnostics (TailSFT's f16 cited verbatim) against a two-parameter empirical-Bayes posterior predictive on released Pass8 rollouts; experiment in `research/cavity_calculus/exp2_passk/`. |
